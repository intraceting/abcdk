...
typedef struct _abcdkipconfig_node
{
    /**/
    abcdkipconfig_t *father;
...
void _abcdkipconfig_node_free(abcdkipconfig_node_t **ctx)
{
    abcdkipconfig_node_t *ctx_p = NULL;
...
abcdkipconfig_node_t *_abcdkipconfig_node_alloc()
{
    abcdkipconfig_node_t *ctx = abcdk_heap_alloc(sizeof(abcdkipconfig_node_t));
...
int _abcdkipconfig_compare_option(abcdk_option_t *a, abcdk_option_t *b)
{
...
void _abcdkipconfig_dump_option(abcdk_option_t *args, abcdk_logger_t *logger)
{
...
void _abcdkipconfig_resolv_hash_code(char buf[33])
{
    abcdk_md5_t *md5_ctx = NULL;
...
void _abcdkipconfig_check_address(abcdkipconfig_node_t *ctx, const char *ifname)
{
    ...
    /*读取当前配置。*/
    _abcdkipconfig_eth_hash_code(curt_addr_hcode, ifname);
...
void _abcdkipconfig_check_resolv(abcdkipconfig_node_t *ctx)
{
    char curt_resolv_hcode[33];

    _abcdkipconfig_resolv_hash_code(curt_resolv_hcode);
...
int _abcdkipconfig_eth_start_udhcpc(abcdkipconfig_node_t *ctx, const char *ifname)
{
    pid_t pid_chk;
...
void _abcdkipconfig_implement(abcdkipconfig_node_t *ctx, abcdk_option_t *opt)
{
    ...
    else if (abcdk_strcmp(enable, "resolv", 0) == 0)
    {
        /*旧的配置只加载一次。*/
        if (!ctx->old_resolv)
            ctx->old_resolv = _abcdkipconfig_resolv_load();

        _abcdkipconfig_check_resolv(ctx);
        ...
        /*记录最新配置。*/
        _abcdkipconfig_resolv_hash_code(ctx->resolv_hcode);
    }
    else
    {
...
WATCHDOG:
    ...
    conf_a = _abcdkipconfig_load_conf(conf_file);
    if (!conf_a)
        goto END;
    ...
    /*判断文件是否正在被修改。*/
    chk = _abcdkipconfig_compare_option(conf_a, conf_b);
    if (chk != 0)
        goto END;
...
END:
    ...
    if (conf_prev)
        _abcdkipconfig_implement(node_ctx, conf_prev);
...
int _abcdkipconfig_daemon_process_cb(void *opaque)
{
    abcdkipconfig_t *ctx = (abcdkipconfig_t *)opaque;

    _abcdkipconfig_process(ctx);
...
int abcdk_tool_ipconfig(abcdk_option_t *args)
{
    ...
    {
        _abcdkipconfig_print_usage(ctx.args);
    }
    else
    {
