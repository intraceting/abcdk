...
static void _abcdkvnet_print_usage(abcdk_option_t *args)
{
    ...
    fprintf(stderr, "\t\t虚拟TUN设备最大传输单元(1400~65535)。默认: 65535\n");
...
static int _abcdkproxy_server_start_listen(abcdkvnet_t *ctx, int ssl_scheme)
{
    ...
    //rpc_cfg.input_mtu = ctx->virtual_tun_mtu+15;
    rpc_cfg.input_mtu = 1400;
...
static void _abcdkvnet_process_server(abcdkvnet_t *ctx)
{
    ...
    ctx->virtual_tun_mtu = abcdk_option_get_int(ctx->args, "--virtual-tun-mtu", 0, 65535);
