...
#
ifeq (${BUILD_REALTIME},)
BUILD_REALTIME := $(shell date +%s)
endif 

#
LIB_DEB_FILENAME = abcdk-lib-${VERSION_STR_FULL}-${BUILD_REALTIME}-${TARGET_MULTIARCH}.deb
LIB_RPM_FILENAME = abcdk-lib-${VERSION_STR_FULL}-${BUILD_REALTIME}-${TARGET_MULTIARCH}.rpm

#
DEV_DEB_FILENAME = abcdk-dev-${VERSION_STR_FULL}-${BUILD_REALTIME}-${TARGET_MULTIARCH}.deb
DEV_RPM_FILENAME = abcdk-dev-${VERSION_STR_FULL}-${BUILD_REALTIME}-${TARGET_MULTIARCH}.rpm

#
ifeq (${INSTALL_NEEDED},yes)
BIN_DEB_FILENAME = abcdk-bin-${VERSION_STR_FULL}-${BUILD_REALTIME}-full-${TARGET_MULTIARCH}.deb
BIN_RPM_FILENAME = abcdk-bin-${VERSION_STR_FULL}-${BUILD_REALTIME}-full-${TARGET_MULTIARCH}.rpm
else
BIN_DEB_FILENAME = abcdk-bin-${VERSION_STR_FULL}-${BUILD_REALTIME}-${TARGET_MULTIARCH}.deb
BIN_RPM_FILENAME = abcdk-bin-${VERSION_STR_FULL}-${BUILD_REALTIME}-${TARGET_MULTIARCH}.rpm
endif
