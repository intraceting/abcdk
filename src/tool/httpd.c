...
typedef struct _abcdk_httpd
{
    ...
    abcdk_hsvr_t *io_ctx;
    abcdk_hsvr_session_t *listen_session;
    abcdk_hsvr_session_t *listen_ssl_session;
...
void _abcdk_httpd_print_usage(abcdk_option_t *args)
{
    ...
    fprintf(stderr, "\n\t--auto-index < 0|1 >\n");
    fprintf(stderr, "\t\t自动索引. 默认: 0\n");
    fprintf(stderr, "\t\t0: 禁用\n");
    fprintf(stderr, "\t\t1: 启用\n");

    fprintf(stderr, "\n\t--enable-h2 < 0|1 >\n");
    fprintf(stderr, "\t\tHTTP2协议. 默认: 1\n");
    fprintf(stderr, "\t\t0: 禁用\n");
    fprintf(stderr, "\t\t1: 启用\n");
}

static void _abcdk_httpd_reply_nobody(abcdk_hsvr_stream_t *stream, int status, const char *acam)
{
    abcdk_httpd_stream_t *stream_ctx_p = (abcdk_httpd_stream_t *)abcdk_hsvr_get_userdata(stream);

    abcdk_hsvr_response_header_set(stream,"Status","%d",status);
    abcdk_hsvr_response_header_set(stream,"Access-Control-Allow-Methods","%s",(acam?acam:"*"));
    abcdk_hsvr_response(stream,NULL);
}

static void _abcdk_httpd_reply_dirent(abcdk_hsvr_stream_t *stream)
{
    abcdk_httpd_stream_t *stream_ctx_p = (abcdk_httpd_stream_t *)abcdk_hsvr_get_userdata(stream);
    ...
    abcdk_hsvr_response_header_set(stream, "Content-Type", "text/html; charset=utf-8");

    abcdk_hsvr_response_format(stream, 10000,
                              "<!DOCTYPE html>\r\n"
                              "<html>\r\n"
...
static void _abcdk_httpd_reply_dirent_more(abcdk_hsvr_stream_t *stream)
{
    abcdk_httpd_stream_t *stream_ctx_p = (abcdk_httpd_stream_t *)abcdk_hsvr_get_userdata(stream);
    char tmp[PATH_MAX], tmp2[PATH_MAX], tmp3[NAME_MAX];
    size_t path_len = PATH_MAX;
    ...
    abcdk_hsvr_response_format(stream, 1000,
                              "</table>"
                              "</pre>\r\n"
                              "<hr>\r\n"
                              "</body>\r\n"
...
static void _abcdk_httpd_reply_file(abcdk_hsvr_stream_t *stream)
{
    abcdk_httpd_stream_t *stream_ctx_p = (abcdk_httpd_stream_t *)abcdk_hsvr_get_userdata(stream);
    ...
    if (stream_ctx_p->range_p)
    {
        ...
        abcdk_hsvr_response_header_set(stream, "Status","%d",206);
        abcdk_hsvr_response_header_set(stream, "Content-Type","%s",type);
        abcdk_hsvr_response_header_set(stream, "Content-Range","bytes %zu-%zu/%zu",range_s, range_e, file_size);
        abcdk_hsvr_response_header_set(stream, "Content-Length","%zu",stream_ctx_p->file_ctx->sizes[0]);
    }
    else
    {
        abcdk_hsvr_response_header_set(stream, "Status","%d",200);
        abcdk_hsvr_response_header_set(stream, "Content-Type","%s",type);
        abcdk_hsvr_response_header_set(stream, "Content-Length","%zu",stream_ctx_p->file_ctx->sizes[0]);
    }
    ...
    if (abcdk_strcmp(stream_ctx_p->method_p, "HEAD", 0) != 0)
        chk = abcdk_hsvr_response(stream, stream_ctx_p->file_ctx);
...
static void _abcdk_httpd_session_prepare_cb(void *opaque, abcdk_hsvr_session_t **session, abcdk_hsvr_session_t *listen)
{
    *session = abcdk_hsvr_session_alloc(((abcdk_httpd_t *)opaque)->io_ctx);
}

static void _abcdk_httpd_session_accept_cb(void *opaque, abcdk_hsvr_session_t *session, int *result)
{
    *result = 0;
}

static void _abcdk_httpd_session_ready_cb(void *opaque, abcdk_hsvr_session_t *session)
{
    abcdk_hsvr_session_set_timeout(session, 30);
}

static void _abcdk_httpd_session_close_cb(void *opaque, abcdk_hsvr_session_t *session)
{
}

static void _abcdk_httpd_stream_destructor_cb(void *opaque, abcdk_hsvr_stream_t *stream)
{
    ...
    abcdk_hsvr_set_userdata(stream, NULL);
}

static void _abcdk_httpd_stream_construct_cb(void *opaque, abcdk_hsvr_stream_t *stream)
{
    abcdk_httpd_t *ctx_p = (abcdk_httpd_t *)opaque;
    ...
    abcdk_hsvr_set_userdata(stream, stream_ctx_p);
}

static void _abcdk_httpd_stream_request_cb(void *opaque, abcdk_hsvr_stream_t *stream)
{
    abcdk_httpd_t *ctx_p = (abcdk_httpd_t *)opaque;
    abcdk_httpd_stream_t *stream_ctx_p = (abcdk_httpd_stream_t *)abcdk_hsvr_get_userdata(stream);
    int chk;
    ...
    chk = abcdk_hsvr_check_auth(stream);
    if (chk != 0)
        return;

    stream_ctx_p->method_p = abcdk_hsvr_request_header_get(stream, "Method");
    stream_ctx_p->scheme_p = abcdk_hsvr_request_header_get(stream, "Scheme");
    stream_ctx_p->host_p = abcdk_hsvr_request_header_get(stream, "Host");
    stream_ctx_p->script_p = abcdk_hsvr_request_header_get(stream, "Script");
    stream_ctx_p->range_p = abcdk_hsvr_request_header_get(stream, "Range");
...
static void _abcdk_httpd_stream_output_cb(void *opaque, abcdk_hsvr_stream_t *stream)
{
    abcdk_httpd_t *ctx_p = (abcdk_httpd_t *)opaque;
    abcdk_httpd_stream_t *stream_ctx_p = (abcdk_httpd_stream_t *)abcdk_hsvr_get_userdata(stream);
    int chk;
...
static int _abcdk_httpd_start_listen(abcdk_httpd_t *ctx,int ssl)
{
    ...
    abcdk_hsvr_config_t cfg = {0};
    abcdk_hsvr_session_t *listen_p;
    int chk;
    ...
    if (ssl)
        listen_p = ctx->listen_ssl_session = abcdk_hsvr_session_alloc(ctx->io_ctx);
    else 
        listen_p = ctx->listen_session = abcdk_hsvr_session_alloc(ctx->io_ctx);
    ...
    chk = abcdk_hsvr_session_listen(listen_p,&listen_addr,&cfg);
    if(chk == 0)
        return 0;
...
static void _abcdk_httpd_process(abcdk_httpd_t *ctx)
{
    ...
    ctx->enable_h2 = abcdk_option_get_int(ctx->args, "--enable-h2",0,1);
...
ERR:
    ...
    abcdk_hsvr_destroy(&ctx->io_ctx);
    abcdk_hsvr_session_unref(&ctx->listen_session);
    abcdk_hsvr_session_unref(&ctx->listen_ssl_session);
