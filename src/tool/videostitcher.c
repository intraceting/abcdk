...
#define ABCDK_TOOL_VIDEOSTITCHER_SRC_MAX 10
    ...
    int src_count;
    const char *src_url_p[ABCDK_TOOL_VIDEOSTITCHER_SRC_MAX];
    ...
    const char *dst_url_p;
    int dst_fps;
    abcdk_xpu_size_t dst_size;

    int quit_if_any_src_on_eof;
    int quit_raise_ok;
...
void _videostitcher_print_usage(abcdk_option_t *args)
{
    ...
    fprintf(stderr, "\n\t--verbose\n");
    fprintf(stderr, ABCDK_GETTEXT("\t 显示详情.\n"));
    ...
    fprintf(stderr, ABCDK_GETTEXT("\t 拼接全景感兴趣区域. 默认: 0, 0, 2147483647, 2147483647\n"));

    fprintf(stderr, "\n\t--dst-video-url < URL >\n");
    fprintf(stderr, ABCDK_GETTEXT("\t 输出视频地址(或文件). 默认: output.mp4 \n"));
    ...
    fprintf(stderr, ABCDK_GETTEXT("\t 有效尺寸: 16x16 ~ 8192x8192 \n"));

    fprintf(stderr, "\n\t--src-video-url < URL >\n");
    fprintf(stderr, ABCDK_GETTEXT("\t 输入视频地址(或文件). \n"));
    ...
    fprintf(stderr, "\n\t--quit-if-any-src-on-eof < 0|1 >\n");
    fprintf(stderr, ABCDK_GETTEXT("\t 任意源到末尾后则立即退出. 默认: 1 \n"));
}
...
void _videostitcher_reader(videostitcher_t *ctx, int id)
{
    ...
    {
        if (abcdk_atomic_compare(&ctx->worker_exit_flag, 1) || abcdk_atomic_compare(&ctx->quit_raise_ok, 1))
            break;

        if (i > 0)
        {
            if (abcdk_atomic_compare(&ctx->quit_if_any_src_on_eof, 1))
            {
                if (abcdk_atomic_compare_and_swap(&ctx->quit_raise_ok, 0, 1))
                    kill(getpid(), SIGQUIT);
                break;
            }
            else
            {
                abcdk_trace_printf(LOG_ERR, ABCDK_GETTEXT("READER[%d], 已到末尾或已断开, 2秒稍后重试."), id);
                sleep(2);
            }
        }
        ...
        if (chk != 0)
        {
            abcdk_trace_printf(LOG_ERR, ABCDK_GETTEXT("READER[%d], 打开源失败, 不存在或无权限."), id);
            continue;
        }
        ...
        while (1)
        {
            ...
            else if (chk > 0)
            {
                ...
                if (undistort_ctx)
                {
                    ...
                    if (chk != 0)
                    {
                        abcdk_trace_printf(LOG_ERR, ABCDK_GETTEXT("READER[%d], 内存不足."), id);
                        break;
                    }
                }
            ...
            {
                ff_stream_eof = 1;
                abcdk_trace_printf(LOG_ERR, ABCDK_GETTEXT("READER[%d], 拉流失败, 已到末尾或已断开."), id);
                continue;
            }
...
END:
    ...
    abcdk_trace_printf(LOG_INFO, ABCDK_GETTEXT("READER[%d], 结束."),id);
...
void _videostitcher_stitching(videostitcher_t *ctx)
{
    ...
    while (1)
    {
        ...
        if (chk != 0)
        {
            abcdk_trace_printf(LOG_ERR, ABCDK_GETTEXT("STITCHING, 全景拼接失败, 内存不足或其它错误."));
            break;
        }
    ...
    abcdk_trace_printf(LOG_INFO, ABCDK_GETTEXT("STITCHING, 结束."));
...
void _videostitcher_writer(videostitcher_t *ctx)
{
    ...
    abcdk_ffmpeg_editor_t *ff_ctx = NULL;
    abcdk_ffmpeg_editor_param_t ff_cfg = {0};
    int ff_stream_idx = -1;
    ...
    for (int i = 0; i < INT32_MAX; i++)
    {
        ...
        if (!ff_ctx)
        {
            if(i>0)
            {
                abcdk_trace_printf(LOG_ERR, ABCDK_GETTEXT("WRITER, 无空间或已断开, 2秒稍后重试."));
                sleep(2);
            }

            ff_ctx = abcdk_ffmpeg_editor_alloc(1);
            if (!ff_ctx)
            {
                abcdk_trace_printf(LOG_ERR, ABCDK_GETTEXT("WRITER, 内存不足."));
                goto END;
            }
            ...
            ff_stream_idx = abcdk_ffmpeg_editor_add_stream_video(ff_ctx, 0, ctx->dst_size.width, ctx->dst_size.height, ctx->dst_fps,
                                                                 enc_params.bitrate, enc_params.level, enc_params.profile, AV_PIX_FMT_YUV420P);
            if (ff_stream_idx < 0)
            {
                abcdk_ffmpeg_editor_free(&ff_ctx);
                abcdk_trace_printf(LOG_ERR, ABCDK_GETTEXT("WRITER, 打开失败, 不存在或无权限."));
                continue;
            }
        }
        ...
        if (chk > 0)
        {
            ...
            chk = abcdk_ffmpeg_editor_write_packet2(ff_ctx, ff_stream_idx, dst_packet->pptrs[0], dst_packet->sizes[0], dst_pts2, dst_pts2);
            if (chk != 0)
            {
                abcdk_ffmpeg_editor_free(&ff_ctx);
                abcdk_trace_printf(LOG_ERR, ABCDK_GETTEXT("WRITER, 写入失败, 无空间或已断开."));
                continue;
            }

            abcdk_trace_printf(LOG_DEBUG, "WRITER, PTS[%lld]", dst_pts2);
...
END:
    ...
    abcdk_trace_printf(LOG_INFO, ABCDK_GETTEXT("WRITER, 结束."));
...
void _videostitcher_work(videostitcher_t *ctx)
{
    ...
    int verbose = abcdk_option_get_int(ctx->args, "--verbose", 0, 0);
    ...
    ctx->dst_url_p = abcdk_option_get(ctx->args, "--dst-video-url", 0, "output.mp4");
    ...
    ctx->quit_if_any_src_on_eof = abcdk_option_get_int(ctx->args, "--quit-if-any-src-on-eof", 0, 1);

    if (verbose)
        abcdk_trace_mask(LOG_EMERG, LOG_ALERT, LOG_CRIT, LOG_ERR, LOG_WARNING, LOG_NOTICE, LOG_INFO, LOG_DEBUG, -1);
    else
        abcdk_trace_mask(LOG_EMERG, LOG_ALERT, LOG_CRIT, LOG_ERR, LOG_WARNING, LOG_NOTICE, LOG_INFO, -1);
    ...
    for (int i = 0; i < ABCDK_ARRAY_SIZE(ctx->src_url_p); i++)
    {
        ctx->src_url_p[i] = abcdk_option_get(ctx->args, "--src-video-url", i, NULL);
    ...
    if (!ctx->dst_url_p || !*ctx->dst_url_p)
    {
        abcdk_trace_printf(LOG_ERR, ABCDK_GETTEXT("输出视频不能缺失."));
        ABCDK_ERRNO_AND_GOTO1(ctx->errcode = -EINVAL, END);
    }
    ...
    ctx->dst_size.height = ABCDK_CLAMP(ctx->dst_size.height, 16, 8192);
