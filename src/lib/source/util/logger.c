...
static int _abcdk_logger_backup(abcdk_logger_t *ctx)
{
    ...
    /*依次修改分段文件编号.*/
    for (int i = ctx->segment_max; i > 0; i--)
    {
        /*编号较大的分段文件.*/
        snprintf(tmp2, PATH_MAX, "%s.%d", ctx->name, i);

        /*删除编号最大的分段文件.*/
        if (i == ctx->segment_max)
        {
        ...
        /*备份编号较小的分段文件.*/
        if (i > 1)
            snprintf(tmp, PATH_MAX, "%s.%d", ctx->name, i - 1);
        else
            strncpy(tmp, ctx->name, PATH_MAX);
...
open_log_file:
    ...
    if (ctx->segment_max > 0 && ctx->segment_size > 0 && attr.st_size >= ctx->segment_size)
    {
        abcdk_closep(&ctx->fd);
        _abcdk_logger_backup(ctx);
        goto open_log_file;
    }
    else
    {
