 ...
 *
 */
#ifndef ABCDK_NVIDIA_JPEG_DECODER_GENERAL_HXX
#define ABCDK_NVIDIA_JPEG_DECODER_GENERAL_HXX
        ...
        namespace jpeg
        {
            ...
            public:
                ...
                virtual abcdk_torch_image_t *update(const void *src, int src_size)
                {
                    ...
                    /*全部转成RGB和GRAY。*/
                    if (components == 3)
                        dst = abcdk_cuda_image_create(width[0], height[0], ABCDK_TORCH_PIXFMT_RGB24, 1);//只使用第一层的宽和高。
                    else if (components == 1)
                        dst = abcdk_cuda_image_create(width[0], height[0], ABCDK_TORCH_PIXFMT_GRAY8, 1);
                    else
                        return NULL;
...
#endif // ABCDK_NVIDIA_JPEG_DECODER_GENERAL_HXX