...
struct _abcdk_rtsp_relay
{
    ...
    /**FF配置. */
    abcdk_ffmpeg_editor_param_t ff_cfg;
...
abcdk_rtsp_relay_t *abcdk_rtsp_relay_create(abcdk_rtsp_server_t *server_ctx, const char *media_name, const char *src_url, const char *src_fmt, int src_timeout, int src_retry)
{
    ...
    ctx = new abcdk_rtsp_relay_t;
    if (!ctx)
        return NULL;

    ctx->exit_flag = 0;
    memset(&ctx->ff_cfg, 0, sizeof(ctx->ff_cfg));
    ctx->ff_ctx = NULL;
    memset(&ctx->worker_thread, 0, sizeof(ctx->worker_thread));
...
static int _abcdk_rtsp_realy_create_media(abcdk_rtsp_relay_t *ctx)
{
    ...
    for (int i = 0; i < abcdk_ffmpeg_editor_stream_nb(ctx->ff_ctx); i++)
    {
        abcdk_object_unref(&extdata);

        src_vs_p = abcdk_ffmpeg_editor_stream_ctx(ctx->ff_ctx, i);

        codecpar = src_vs_p->codecpar;

        /*获取流的帧率作为缓存长度.*/
        cache = abcdk_ffmpeg_editor_stream_time2rate(ctx->ff_ctx, i);
        /*码率转换(bps -> kbps).*/
...
static void _abcdk_rtsp_realy_forward_media(abcdk_rtsp_relay_t *ctx)
{
    ...
    while (!ctx->exit_flag)
    {
        chk = abcdk_ffmpeg_editor_read_packet(ctx->ff_ctx, &pkt);
        if (chk != 0)
            break;

        src_idx = pkt.stream_index;
...
static void *_abcdk_rtsp_relay_worker_thread_routine(void *opaque)
{
    ...
    /*设置线程名字, 日志记录会用到.*/
    abcdk_thread_setname(pthread_self(), "RTSP-RE-%x", abcdk_sequence_num());
