...
RETRY:
    ...
    if (abcdk_strncmp(ctx->src_url, "sock://", 7, 0) == 0)
    {
        ctx->ff_cfg.io.read_cb = abcdk_ffsocket_read;
        ctx->ff_cfg.io.opaque = ctx->ff_sock = abcdk_ffsocket_create(ctx->src_url + 7, ctx->src_timeout, NULL, NULL, NULL);

        if (!ctx->ff_sock)
            goto RETRY;

        ctx->ff_cfg.fmt = (ctx->src_fmt ? ctx->src_fmt : "mpegts");
    }
    else
    {
