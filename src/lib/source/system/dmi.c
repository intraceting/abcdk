...
int abcdk_dmi_hash(uint8_t uuid[16], const char *stuff)
{
    ...
    if(chk != 0)
    {
        abcdk_trace_printf(LOG_WARNING,ABCDK_GETTEXT("打开目录(%s)失败，无权限或不存在，忽略。"),"/dev/disk/by-uuid");
    }

    chk = abcdk_dirent_open(&dir_ctx,"/dev/disk/by-partuuid");
    if(chk != 0)
    {
        abcdk_trace_printf(LOG_WARNING,ABCDK_GETTEXT("打开目录(%s)失败，无权限或不存在，忽略。"),"/dev/disk/by-partuuid");
    }

    chk = abcdk_dirent_open(&dir_ctx,"/dev/disk/by-id");
    if(chk != 0)
    {
        abcdk_trace_printf(LOG_WARNING,ABCDK_GETTEXT("打开目录(%s)失败，无权限或不存在，忽略。"),"/dev/disk/by-id");
    }
    ...
    while(1)
    {
        ...
        abcdk_trace_printf(LOG_DEBUG,ABCDK_GETTEXT("块设备分区的别名(%s)将被用于计算DMI的哈希值。"),alias_name);
    ...
    if(!p)
    {
        abcdk_trace_printf(LOG_ERR,ABCDK_GETTEXT("没有发现可用于计算DMI哈希值的固定块设备"));
