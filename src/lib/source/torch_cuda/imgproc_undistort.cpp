                                                ...
                                                const double camera_matrix[3][3], const double dist_coeffs[5])
{
    ...
    chk = abcdk_torch_image_reset_cuda(xmap, tmp_xmap.cols, tmp_xmap.rows, ABCDK_TORCH_PIXFMT_GRAYF32, 1);
    if (chk != 0)
        return -2;

    chk = abcdk_torch_image_reset_cuda(ymap, tmp_ymap.cols, tmp_ymap.rows, ABCDK_TORCH_PIXFMT_GRAYF32, 1);
    if (chk != 0)
        return -3;

    abcdk_torch_image_copy_plane_cuda(*xmap, 0, tmp_xmap.data, tmp_xmap.step);
    abcdk_torch_image_copy_plane_cuda(*ymap, 0, tmp_ymap.data, tmp_ymap.step);
