...
template <typename T>
ABCDK_TORCH_INVOKE_GLOBAL void _abcdk_torch_imgproc_compose_2d2d_cuda(int channels, bool packed,
                                                                ...
                                                                uint32_t *scalar, size_t overlap_x, size_t overlap_y, size_t overlap_w, bool optimize_seam)
{
    ...
    abcdk::torch::imageproc::compose<T>(channels, packed,
...
template <typename T>
ABCDK_TORCH_INVOKE_HOST int _abcdk_torch_imgproc_compose_cuda(int channels, bool packed,
                                                        T *panorama, size_t panorama_w, size_t panorama_ws, size_t panorama_h,
                                                        T *compose, size_t compose_w, size_t compose_ws, size_t compose_h,
                                                        uint32_t *scalar, size_t overlap_x, size_t overlap_y, size_t overlap_w, bool optimize_seam)
{
    ...
    /*2D-2D*/
    abcdk::torch_cuda::grid::make_dim_dim(dim, compose_w * compose_h, 64);
