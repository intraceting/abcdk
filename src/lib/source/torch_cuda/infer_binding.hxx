        ...
        namespace infer
        {
            ...
            public:
                ...
                int prepare(int input, int index, const char *name, nvinfer1::DataType type,const nvinfer1::Dims &dims, abcdk_option_t *opt)
                {
                    ...
                    const char *tensor_dims_p = abcdk_option_get(opt, "--input-tensor-dims", 0, "0,1,2,3");
                    ...
                    m_tensor_b_size = m_dims.d[tensor_b_index];
                    m_tensor_c_size = m_dims.d[tensor_c_index];
                    m_tensor_h_size = m_dims.d[tensor_h_index];
                    m_tensor_w_size = m_dims.d[tensor_w_index];

                    assert(m_tensor_b_size >= 1 && m_tensor_c_size == 3 && m_tensor_h_size > 0 && m_tensor_w_size > 0);
                    ...
                    {
                        abcdk_torch_image_reset_cuda(&m_input_img_cache[i], m_tensor_w_size, m_tensor_h_size, ABCDK_TORCH_PIXFMT_RGB24, 1);
