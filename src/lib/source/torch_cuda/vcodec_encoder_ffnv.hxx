 ...
 *
 */
#ifndef ABCDK_TORCH_NVIDIA_VCODEC_ENCODER_FFNV_HXX
            ...
            public:
                ...
                virtual int update(abcdk_torch_packet_t **dst, const abcdk_torch_frame_t *src)
                {
                    ...
                    if (src)
                    {
                        ...
                        {
                            tmp_src = abcdk_torch_frame_create_cuda(src->img->width, src->img->height, ABCDK_TORCH_PIXFMT_RGB32, 1);
                            if (!tmp_src)
                                return -1;
                            
                            /*转换格式。*/
                            chk = abcdk_torch_image_convert_cuda(tmp_src->img, src->img); 
...
#endif // ABCDK_TORCH_NVIDIA_VCODEC_ENCODER_FFNV_HXX