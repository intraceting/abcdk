...
namespace bytetrack
{
    ...
    BYTETracker::BYTETracker(int _max_time_lost, float _track_thresh /*= 0.5*/, float _high_thresh /*= 0.6*/, float _match_thresh /*= 0.8*/)
    {
    ...
    BYTETracker::BYTETracker(int frame_rate, int track_buffer)
        : BYTETracker(frame_rate / 30.0 * track_buffer)
    {
    }
    ...
    vector<STrack> BYTETracker::update(const vector<object> &objects)
    {
        ...
        if (objects.size() > 0)
        {
            for (int i = 0; i < objects.size(); i++)
            {
                ...
                uint64_t magic = objects[i].magic;

                STrack strack(STrack::tlbr_to_tlwh(tlbr_), score, magic);
