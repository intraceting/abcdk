...
template <typename T>
ABCDK_TORCH_INVOKE_HOST void _abcdk_torch_imgproc_compose_1d_host(int channels, bool packed,
...
template <typename T>
ABCDK_TORCH_INVOKE_HOST int _abcdk_torch_imgproc_compose_host(int channels, bool packed,
                                                        T *panorama, size_t panorama_w, size_t panorama_ws, size_t panorama_h,
                                                        T *compose, size_t compose_w, size_t compose_ws, size_t compose_h,
                                                        uint32_t *scalar, size_t overlap_x, size_t overlap_y, size_t overlap_w, bool optimize_seam)
{
    ...
    _abcdk_torch_imgproc_compose_1d_host<T>(channels, packed, panorama, panorama_w, panorama_ws, panorama_h,
                                            compose, compose_w, compose_ws, compose_h,
                                     ...
                                     uint32_t scalar[], size_t overlap_x, size_t overlap_y, size_t overlap_w, int optimize_seam)
{
    ...
    assert(panorama->tag == ABCDK_TORCH_TAG_HOST);
    assert(compose->tag == ABCDK_TORCH_TAG_HOST);

    dst_depth = abcdk_torch_pixfmt_channels(panorama->pixfmt);

    return _abcdk_torch_imgproc_compose_host<uint8_t>(dst_depth, true,
