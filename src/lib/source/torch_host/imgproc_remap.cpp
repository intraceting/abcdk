...
static int _abcdk_torch_imgproc_remap_8u_host(int channels, int packed,
                                   ...
                                   int inter_mode)
{
    ...
    assert(xmap->tag == ABCDK_TORCH_TAG_HOST);
    assert(ymap->tag == ABCDK_TORCH_TAG_HOST);

    dst_depth = abcdk_torch_pixfmt_channels(dst->pixfmt);

    return _abcdk_torch_imgproc_remap_8u_host(dst_depth, true,
