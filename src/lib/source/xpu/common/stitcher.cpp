...
namespace abcdk_xpu
{
    namespace common
    {
        class stitcher_impl : public stitcher
        {
        private:
        ...
        public:
            stitcher_impl()
            {
                ...
                set_feature_matcher("Best", 0.3);
                set_estimator("Homography");
                set_bundle_adjuster("ray");
                set_warper("spherical", 1.0);
            }

            virtual ~stitcher_impl()
            {
            }
        ...
        protected:
            void find_feature(const std::vector<cv::Mat> &imgs, const std::vector<cv::Mat> &masks)
            {
                ...
                ABCDK_TRACE_ASSERT(m_feature_finder.get() != NULL, ABCDK_GETTEXT("未指定特征查找算法."));
            ...
            bool match_feature()
            {
                ABCDK_TRACE_ASSERT(m_feature_matcher.get() != NULL, ABCDK_GETTEXT("未指定特征匹配算法."));
            ...
            {
                ABCDK_TRACE_ASSERT(m_estimator.get() != NULL, ABCDK_GETTEXT("未指定相机参数估计算法."));
            ...
            {
                ABCDK_TRACE_ASSERT(m_bundle_adjuster.get() != NULL, ABCDK_GETTEXT("未指定相机参数调节算法."));
            ...
            bool panorama_param_correct()
            {
                ...
                ABCDK_TRACE_ASSERT(m_rotation_warper.get() != NULL, ABCDK_GETTEXT("未指定图像变换算法."));
                ...
                std::vector<cv::Rect> screen_rects(m_warper_rects.size());

                /*相机坐标转屏幕坐标.*/
                for (int i = 0; i < m_warper_rects.size(); i++)
                {
                    screen_rects[i] = m_warper_rects[i];

                    if (min_x < 0)
                        screen_rects[i].x += abs(min_x);
                    else
                        screen_rects[i].x -= min_x;

                    if (min_y < 0)
                        screen_rects[i].y += abs(min_y);
                    else
                        screen_rects[i].y -= min_y;
                }

                m_panorama_size.width = 0;
                m_panorama_size.height = 0;

                /*计算拼接后图像的最大宽和高.*/
                for (int i = 0; i < screen_rects.size(); i++)
                {
                    if (m_panorama_size.width < screen_rects[i].x + screen_rects[i].width)
                        m_panorama_size.width = screen_rects[i].x + screen_rects[i].width;

                    if (m_panorama_size.height < screen_rects[i].y + screen_rects[i].height)
                        m_panorama_size.height = screen_rects[i].y + screen_rects[i].height;
                }

                m_blend_idxs = m_img_good_idxs;
                m_blend_rects = screen_rects;
        ...
        public:
            ...
            int set_feature_matcher(const char *name, float threshold = 0.3)
            {
                assert(name != NULL);

                if (strcasecmp(name, "Range") == 0)
                    set_feature_matcher(cv::makePtr<cv::detail::BestOf2NearestRangeMatcher>(5, false, threshold));
                else if (strcasecmp(name, "Affine") == 0)
                    set_feature_matcher(cv::makePtr<cv::detail::AffineBestOf2NearestMatcher>(false, false, threshold));
                else if (strcasecmp(name, "Best") == 0)
                    set_feature_matcher(cv::makePtr<cv::detail::BestOf2NearestMatcher>(false, threshold));
                else
                    return -1;
            ...
            int estimate_parameters(const std::vector<cv::Mat> &imgs, const std::vector<cv::Mat> &masks,
            ...
            int build_parameters()
            {
                ABCDK_TRACE_ASSERT(!m_panorama_param_ok, ABCDK_GETTEXT("全景参数已经构建完成, 不能重复构建."));
            ...
            int dump_parameters(std::string &dst, const char *magic /*= NULL*/)
            {
                ...
                cv::write(f, "good_count", (int)m_img_good_idxs.size());
                ...
                for (int i = 0; i < m_img_good_idxs.size(); i++)
                {
                    ...
                    cv::write(f, key, m_img_good_idxs[i]);
                }
                ...
                for (int i = 0; i < m_img_good_sizes.size(); i++)
                {
                    ...
                    cv::write(f, key, m_img_good_sizes[i]);
                }
                ...
                for (int i = 0; i < m_camera_params.size(); i++)
                {
                    ...
                    key += std::to_string(i);
                    key += "_focal";
                    cv::write(f, key, m_camera_params[i].focal);

                    key = "camera_params_";
                    key += std::to_string(i);
                    key += "_aspect";
                    cv::write(f, key, m_camera_params[i].aspect);

                    key = "camera_params_";
                    key += std::to_string(i);
                    key += "_ppx";
                    cv::write(f, key, m_camera_params[i].ppx);

                    key = "camera_params_";
                    key += std::to_string(i);
                    key += "_ppy";
                    cv::write(f, key, m_camera_params[i].ppy);

                    key = "camera_params_";
                    key += std::to_string(i);
                    key += "_R";
                    cv::write(f, key, m_camera_params[i].R);

                    key = "camera_params_";
                    key += std::to_string(i);
                    key += "_t";
                    cv::write(f, key, m_camera_params[i].t);
                }
            ...
            int load_parameters(const char *src, const char *magic /*= NULL*/)
            {
                ...
                m_img_good_idxs.resize(good_count);
                m_img_good_sizes.resize(good_count);
                m_camera_params.resize(good_count);
                ...
                for (int i = 0; i < m_img_good_sizes.size(); i++)
                {
                    ...
                    node = f[key.c_str()];
                    m_img_good_sizes[i] = cv::Size(node[0], node[1]);
                }
                ...
                for (int i = 0; i < m_camera_params.size(); i++)
                {
                    ...
                    node = f[key.c_str()];
                    m_camera_params[i].t = node.mat();
                }

                m_panorama_param_ok = false; // Not OK.
                m_camera_param_ok = true;    // OK.
        ...
        std::shared_ptr<stitcher> stitcher::create()
        {
            return std::make_shared<stitcher_impl>();
        }
