 ...
 *
 */
#ifndef ABCDK_XPU_COMMON_DNN_YOLO_V11_SEG_HXX
#define ABCDK_XPU_COMMON_DNN_YOLO_V11_SEG_HXX
    ...
    namespace common
    {
        namespace dnn
        {
            ...
            protected:
                ...
                {
                    abcdk_xpu_dnn_tensor_t *input_tensor_p = NULL, *output_tensor_p = NULL,*output2_tensor_p = NULL;
                    
                    for (auto &t : tensor)
                    {
                        if (t.mode == 1)
                        {
                            ABCDK_TRACE_ASSERT(input_tensor_p == NULL, ABCDK_GETTEXT("YOLO-11模型仅支持一个输入层."));
                            input_tensor_p = &t;
                        }
                        else if (t.mode == 2)
                        {
                            if(t.dims.nb == 3)
                            {
                                ABCDK_TRACE_ASSERT(output_tensor_p == NULL, ABCDK_GETTEXT("YOLO-11模型仅支持一个输出层(nb==3)."));
                                assert(output_tensor_p == NULL);
                                output_tensor_p = &t;
                            }
                            else if(t.dims.nb == 4)
                            {
                                ABCDK_TRACE_ASSERT(output2_tensor_p == NULL, ABCDK_GETTEXT("YOLO-11模型仅支持一个输出层(nb==4)."));
                    ...
                    ABCDK_TRACE_ASSERT(input_tensor_p != NULL, ABCDK_GETTEXT("YOLO-11模型至少存在一个输入层."));
                    ABCDK_TRACE_ASSERT(output_tensor_p != NULL, ABCDK_GETTEXT("YOLO-11模型至少存在一个输出层(nb==3)."));
                    ABCDK_TRACE_ASSERT(output2_tensor_p != NULL, ABCDK_GETTEXT("YOLO-11模型至少存在一个输出层(nb==4)."));
                    ...
                    ndarray output2_data((void *)output2_tensor_p->data_p, false, output2_b, output2_c, output2_h, output2_w, output2_w * sizeof(float));
                    ...
                    for (int b = 0; b < output_b; b++)
                    {
                        for (int y = 0; y < output_h; y++)
                        {
                            ...
                            float cx = output_data.obj<float>(b, 0, y, 0); // 矩型中心点X坐标.
                            float cy = output_data.obj<float>(b, 0, y, 1); // 矩型中心点Y坐标.
...
#endif // ABCDK_XPU_COMMON_DNN_YOLO_V11_SEG_HXX