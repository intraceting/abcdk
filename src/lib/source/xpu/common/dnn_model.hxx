    ...
    namespace common
    {
        namespace dnn
        {
            ...
            public:
                ...
                virtual void process(std::vector<abcdk_xpu_dnn_tensor_t> &tensor, float score_threshold, float nms_threshold)
                {
                ...
                virtual void fetch(std::vector<abcdk_xpu_dnn_object_t> &dst, int index)
                {
                    ABCDK_TRACE_ASSERT(index < m_object.size(), ABCDK_GETTEXT("超出图像批量范围."));
            ...
            protected:
                 ...
                 */
                virtual void collect_object(std::vector<std::map<int, std::vector<object>>> &dst, std::vector<abcdk_xpu_dnn_tensor_t> &tensor, float threshold) = 0;
...
#endif // ABCDK_XPU_COMMON_DNN_MODEL_HXX