    ...
    namespace general
    {
        namespace calibrate
        {
            ...
            int build_parameters(metadata_t *ctx, double alpha)
            {
                ...
                ctx->warper_xmap = image::clone(ABCDK_XPU_PIXFMT_GRAYF32, ctx->co_ctx->m_warper_xmap, 16);
                if (!ctx->warper_xmap)
                    return -ENOMEM;

                ctx->warper_ymap = image::clone(ABCDK_XPU_PIXFMT_GRAYF32, ctx->co_ctx->m_warper_ymap, 16);
                if (!ctx->warper_ymap)
                    return -ENOMEM;
            ...
            static int _undistort(metadata_t *ctx, const image::metadata_t *src, image::metadata_t **dst, abcdk_xpu_inter_t inter_mode)
            {
                ...
                chk = image::reset(dst, src->width, src->height, pixfmt::ffmpeg_to_local(src->format), 16);
                if (chk != 0)
                    return chk;
