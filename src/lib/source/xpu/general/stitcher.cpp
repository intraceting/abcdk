...
namespace abcdk_xpu
{
    namespace general
    {
        namespace stitcher
        {
            ...
            {
                std::vector<cv::Mat> tmp_img(count);
                std::vector<cv::Mat> tmp_mask(count);
            ...
            int build_parameters(metadata_t *ctx)
            {
                ...
                {
                    ctx->warper_xmaps[i] = image::clone(ABCDK_XPU_PIXFMT_GRAYF32, ctx->co_ctx->m_warper_xmaps[i], 16);
                    if (!ctx->warper_xmaps[i])
                        return -ENOMEM;

                    ctx->warper_ymaps[i] = image::clone(ABCDK_XPU_PIXFMT_GRAYF32, ctx->co_ctx->m_warper_ymaps[i], 16);
            ...
            static int _compose(metadata_t *ctx, int count, const image::metadata_t *img[], image::metadata_t **out, int optimize_seam)
            {
                ...
                for (int i = 0; i < ctx->co_ctx->m_img_good_idxs.size(); i++)
                {
                    ...
                    chk = image::reset(&ctx->warper_parts[idx], warper_w, warper_h, pixfmt::ffmpeg_to_local(img_p->format), 16);
                    if (chk != 0)
                        return chk;
                ...
                chk = image::reset(out, ctx->co_ctx->m_panorama_size.width, ctx->co_ctx->m_panorama_size.height,
                                   pixfmt::ffmpeg_to_local(ctx->warper_parts[ctx->co_ctx->m_img_good_idxs[0]]->format), 16);
