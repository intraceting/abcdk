    ...
    namespace nvidia
    {
        namespace imgproc
        {
            static int _resize(const image::metadata_t *src, const abcdk_xpu_rect_t *src_roi, image::metadata_t *dst, abcdk_xpu_inter_t inter_mode)
            {
                ...
                {
                    npp_chk = nppiResize_8u_C1R(src->data[0], src->linesize[0], tmp_src_size, tmp_src_roi,
                ...
                {
                    npp_chk = nppiResize_8u_C3R(src->data[0], src->linesize[0], tmp_src_size, tmp_src_roi,
                                                dst->data[0], dst->linesize[0], tmp_dst_size, tmp_dst_roi,
                                                inter_local_to_nppi(inter_mode));
                }
                else if (src->format == AV_PIX_FMT_RGB32 || src->format == AV_PIX_FMT_BGR32)
                {
                    npp_chk = nppiResize_8u_C4R(src->data[0], src->linesize[0], tmp_src_size, tmp_src_roi,
                                                dst->data[0], dst->linesize[0], tmp_dst_size, tmp_dst_roi,
                                                inter_local_to_nppi(inter_mode));
                }
                else if (src->format == AV_PIX_FMT_GRAYF32)
                {
                    npp_chk = nppiResize_32f_C1R((Npp32f *)src->data[0], src->linesize[0], tmp_src_size, tmp_src_roi,
