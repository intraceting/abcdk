...
/**HTTP节点.*/
typedef struct _abcdk_hsvr_node
{
    /*父级.*/
    abcdk_hsvr_t *father;

    /*配置.*/
    abcdk_hsvr_config_t cfg;
...
/*流.*/
typedef struct _abcdk_hsvr_stream_internal
{
    /*流ID.*/
    int id;
...
}abcdk_hsvr_stream_internal_t;

static int64_t _abcdk_hsvr_clock(uint8_t precision)
{
    return abcdk_time_clock2kind_with(CLOCK_MONOTONIC, precision);
}

static void _abcdk_hsvr_stream_destructor_cb(abcdk_object_t *obj, void *opaque)
{
    abcdk_stcp_node_t *io_node_p;
    abcdk_hsvr_node_t *node_ctx_p;
    abcdk_hsvr_stream_internal_t *stream_ctx_p;

    io_node_p = (abcdk_stcp_node_t *)opaque;
    node_ctx_p = (abcdk_hsvr_node_t *)abcdk_stcp_get_userdata(io_node_p);
    stream_ctx_p = (abcdk_hsvr_stream_internal_t *)obj->pptrs[ABCDK_MAP_VALUE];

    /*通知流执行析构.*/
    if(node_ctx_p->cfg.stream_destructor_cb)
        node_ctx_p->cfg.stream_destructor_cb(node_ctx_p->cfg.opaque,(abcdk_hsvr_stream_t *)obj);
...
static void _abcdk_hsvr_stream_construct_cb(abcdk_object_t *obj, void *opaque)
{
    abcdk_stcp_node_t *io_node_p;
    abcdk_hsvr_node_t *node_ctx_p;
    abcdk_hsvr_stream_internal_t *stream_ctx_p;
    ...
    if(node_ctx_p->cfg.stream_construct_cb)
        node_ctx_p->cfg.stream_construct_cb(node_ctx_p->cfg.opaque,(abcdk_hsvr_stream_t *)obj);
}

static void _abcdk_hsvr_stream_remove_cb(abcdk_object_t *stream, void *opaque)
{
    abcdk_stcp_node_t *io_node_p;
    abcdk_hsvr_node_t *node_ctx_p;
    abcdk_hsvr_stream_internal_t *stream_ctx_p;
    ...
    io_node_p = (abcdk_stcp_node_t *)opaque;
    node_ctx_p = (abcdk_hsvr_node_t *)abcdk_stcp_get_userdata(io_node_p);
    stream_ctx_p = (abcdk_hsvr_stream_internal_t *)stream->pptrs[ABCDK_MAP_VALUE];

    /*通知流已关闭.*/
    if(node_ctx_p->cfg.stream_close_cb)
        node_ctx_p->cfg.stream_close_cb(node_ctx_p->cfg.opaque,(abcdk_hsvr_stream_t *)stream);
}

static void _abcdk_hsvr_node_destroy_cb(void *userdata)
{
    abcdk_hsvr_node_t *ctx;

    if (!userdata)
        return;

    ctx = (abcdk_hsvr_node_t *)userdata;
...
static void _abcdk_hsvr_log(abcdk_object_t *stream, uint32_t status)
{
    abcdk_hsvr_node_t *node_ctx_p;
    abcdk_hsvr_stream_internal_t *stream_ctx_p;
    ...
    stream_ctx_p = (abcdk_hsvr_stream_internal_t *)stream->pptrs[ABCDK_MAP_VALUE];
    node_ctx_p = (abcdk_hsvr_node_t *)abcdk_stcp_get_userdata(stream_ctx_p->io_node);
...
static void _abcdk_hsvr_process(abcdk_object_t *stream)
{
    abcdk_hsvr_node_t *node_ctx_p;
    abcdk_hsvr_stream_internal_t *stream_ctx_p;
    ...
    _abcdk_hsvr_log(stream,0);

    /*通知应用层数据到达.*/
    node_ctx_p->cfg.stream_request_cb(node_ctx_p->cfg.opaque,(abcdk_hsvr_stream_t *)stream);

}

#ifdef NGHTTP2_H

static void _abcdk_hsvr_process_2(abcdk_object_t *stream)
{
    abcdk_hsvr_node_t *node_ctx_p;
    abcdk_hsvr_stream_internal_t *stream_ctx_p;
    ...
    _abcdk_hsvr_process(stream);
}

static int _abcdk_hsvr_h2_frame_recv_cb(nghttp2_session *session, const nghttp2_frame *frame, void *user_data)
{
    abcdk_stcp_node_t *node = (abcdk_stcp_node_t *)user_data;
    abcdk_hsvr_node_t *node_ctx_p = (abcdk_hsvr_node_t *)abcdk_stcp_get_userdata(node);
    abcdk_hsvr_stream_internal_t *stream_ctx_p;
    ...
    stream_ctx_p = (abcdk_hsvr_stream_internal_t *)stream_p->pptrs[ABCDK_MAP_VALUE];
    ...
    _abcdk_hsvr_process_2(stream_p);
...
static int _abcdk_hsvr_h2_data_chunk_recv_cb(nghttp2_session *session, uint8_t flags, int32_t stream_id, const uint8_t *data, size_t len, void *user_data)
{
    abcdk_stcp_node_t *node = (abcdk_stcp_node_t *)user_data;
    abcdk_hsvr_node_t *node_ctx_p = (abcdk_hsvr_node_t *)abcdk_stcp_get_userdata(node);
    abcdk_hsvr_stream_internal_t *stream_ctx_p;
...
static int _abcdk_hsvr_h2_header_cb(nghttp2_session *session, const nghttp2_frame *frame,
...
static int _abcdk_hsvr_h2_begin_headers_cb(nghttp2_session *session, const nghttp2_frame *frame, void *user_data)
{
    abcdk_stcp_node_t *node = (abcdk_stcp_node_t *)user_data;
    abcdk_hsvr_node_t *node_ctx_p = (abcdk_hsvr_node_t *)abcdk_stcp_get_userdata(node);
    abcdk_hsvr_stream_internal_t *stream_ctx_p;
    ...
    stream_p = abcdk_map_find2(node_ctx_p->stream_map, &stream_id, sizeof(abcdk_hsvr_stream_internal_t));
    if (!stream_p)
        return -1;
...
static int _abcdk_hsvr_h2_stream_close_cb(nghttp2_session *session, int32_t stream_id, uint32_t error_code, void *user_data)
{
    abcdk_stcp_node_t *node = (abcdk_stcp_node_t *)user_data;
    abcdk_hsvr_node_t *node_ctx_p = (abcdk_hsvr_node_t *)abcdk_stcp_get_userdata(node);
...
static ssize_t _abcdk_hsvr_h2_send_cb(nghttp2_session *session, const uint8_t *data, size_t length, int flags, void *user_data)
{
    abcdk_stcp_node_t *node = (abcdk_stcp_node_t *)user_data;
    abcdk_hsvr_node_t *node_ctx_p = (abcdk_hsvr_node_t *)abcdk_stcp_get_userdata(node);
    int chk;
    ...
    /*记录活动时间.*/
    node_ctx_p->h2_send_active = _abcdk_hsvr_clock(0);
...
static ssize_t _abcdk_hsvr_h2_response_read_cb(nghttp2_session *session, int32_t stream_id, uint8_t *buf, size_t length,
                                                uint32_t *data_flags, nghttp2_data_source *source, void *user_data)
{
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        if (node_ctx_p->cfg.stream_output_cb)
            node_ctx_p->cfg.stream_output_cb(node_ctx_p->cfg.opaque, (abcdk_hsvr_stream_t *)stream_p);
    }
...
static void _abcdk_hsvr_process_1(abcdk_object_t *stream)
{
    abcdk_hsvr_node_t *node_ctx_p;
    abcdk_hsvr_stream_internal_t *stream_ctx_p;
    const char *line_p;
    ...
    if (stream_ctx_p->protocol == 1)
    {
        ...
        if (node_ctx_p->ssl_scheme == ABCDK_STCP_SSL_SCHEME_PKI)
            stream_ctx_p->scheme = abcdk_object_copyfrom("hsvr", 5);
        else
...
static void _abcdk_hsvr_request_1(abcdk_stcp_node_t *node, const void *data, size_t size, size_t *remain)
{
    abcdk_hsvr_node_t *node_ctx_p;
    abcdk_object_t *stream_p;
    abcdk_hsvr_stream_internal_t *stream_ctx_p;
    ...
    node_ctx_p = (abcdk_hsvr_node_t *)abcdk_stcp_get_userdata(node);
    ...
    *remain = 0;
    ...
    /**/
    _abcdk_hsvr_process_1(stream_p);
...
static void _abcdk_hsvr_request_2(abcdk_stcp_node_t *node, const void *data, size_t size, size_t *remain)
{
    abcdk_hsvr_node_t *node_ctx_p;
    ssize_t chk;
...
static void _abcdk_hsvr_output_1(abcdk_stcp_node_t *node)
{
    abcdk_hsvr_node_t *node_ctx_p;
    abcdk_object_t *stream_p;
    abcdk_hsvr_stream_internal_t *stream_ctx_p;
...
static void _abcdk_hsvr_output_2(abcdk_stcp_node_t *node)
{
    abcdk_hsvr_node_t *node_ctx_p;
    int chk;
    ...
    /*记录活动时间.*/
    node_ctx_p->h2_send_want = _abcdk_hsvr_clock(0);
...
static void _abcdk_hsvr_prepare_cb(abcdk_stcp_node_t **node, abcdk_stcp_node_t *listen)
{
    abcdk_stcp_node_t *node_p;
    abcdk_hsvr_node_t *listen_ctx_p, *node_ctx_p;
    int chk;

    listen_ctx_p = (abcdk_hsvr_node_t *)abcdk_stcp_get_userdata(listen);

    listen_ctx_p->cfg.session_prepare_cb(listen_ctx_p->cfg.opaque,(abcdk_hsvr_session_t **)&node_p,(abcdk_hsvr_session_t *)listen);
    if (!node_p)
        return;

    node_ctx_p = (abcdk_hsvr_node_t *)abcdk_stcp_get_userdata(node_p);
...
static void _abcdk_hsvr_event_accept(abcdk_stcp_node_t *node, int *result)
{
    abcdk_hsvr_node_t *node_ctx_p;
    int chk;
    ...
    *result = 0;
    ...
    if(node_ctx_p->cfg.session_accept_cb)
        node_ctx_p->cfg.session_accept_cb(node_ctx_p->cfg.opaque, (abcdk_hsvr_session_t*)node, result);
    
    if(*result != 0)
        abcdk_trace_printf(LOG_WARNING, ABCDK_GETTEXT("禁止客户端(%s)连接到本机(%s)."), node_ctx_p->remote_addr, node_ctx_p->local_addr);
}

static void _abcdk_hsvr_event_connect(abcdk_stcp_node_t *node)
{
    abcdk_hsvr_node_t *node_ctx_p;
    char proto[256] = {0};
    int chk;
    ...
    if (node_ctx_p->protocol == 2)
    {
        ...
        nghttp2_session_callbacks_set_on_frame_recv_callback(node_ctx_p->h2_cbs, _abcdk_hsvr_h2_frame_recv_cb);
        nghttp2_session_callbacks_set_on_data_chunk_recv_callback(node_ctx_p->h2_cbs, _abcdk_hsvr_h2_data_chunk_recv_cb);
        nghttp2_session_callbacks_set_on_header_callback(node_ctx_p->h2_cbs, _abcdk_hsvr_h2_header_cb);
        nghttp2_session_callbacks_set_on_begin_headers_callback(node_ctx_p->h2_cbs, _abcdk_hsvr_h2_begin_headers_cb);
        nghttp2_session_callbacks_set_on_stream_close_callback(node_ctx_p->h2_cbs, _abcdk_hsvr_h2_stream_close_cb);
        nghttp2_session_callbacks_set_send_callback(node_ctx_p->h2_cbs, _abcdk_hsvr_h2_send_cb);

        nghttp2_session_server_new(&node_ctx_p->h2_handle, node_ctx_p->h2_cbs, node);

        nghttp2_seabcdk_gettextings_entry iv[] = {
            {NGHTTP2_SEABCDK_GETTEXTINGS_MAX_CONCURRENT_STREAMS, 100},
            {NGHTTP2_SEABCDK_GETTEXTINGS_MAX_HEADER_LIST_SIZE, 100},
            {NGHTTP2_SEABCDK_GETTEXTINGS_MAX_FRAME_SIZE, 65535},
            {NGHTTP2_SEABCDK_GETTEXTINGS_INITIAL_WINDOW_SIZE, 16 * 1024 * 1024}};

        /*必须要设置.*/
        nghttp2_submit_seabcdk_gettextings(node_ctx_p->h2_handle, NGHTTP2_FLAG_NONE, iv, 4);
#endif // NGHTTP2_H
    }

    if(node_ctx_p->cfg.session_ready_cb)
        node_ctx_p->cfg.session_ready_cb(node_ctx_p->cfg.opaque, (abcdk_hsvr_session_t*)node);
...
static void _abcdk_hsvr_event_output(abcdk_stcp_node_t *node)
{
    abcdk_hsvr_node_t *node_ctx_p;
    ...
    {
        _abcdk_hsvr_output_1(node);
    }
    else if( node_ctx_p->protocol == 2)
    {
        _abcdk_hsvr_output_2(node);
    }
}

static void _abcdk_hsvr_event_close(abcdk_stcp_node_t *node)
{
    abcdk_hsvr_node_t *node_ctx_p;
    int chk;
    ...
    if(node_ctx_p->cfg.session_close_cb)
        node_ctx_p->cfg.session_close_cb(node_ctx_p->cfg.opaque,(abcdk_hsvr_session_t*)node);
}

static void _abcdk_hsvr_event_cb(abcdk_stcp_node_t *node, uint32_t event, int *result)
{
    abcdk_hsvr_node_t *node_ctx_p;
    int chk;
    ...
    {
        _abcdk_hsvr_event_accept(node,result);
    }
    else if (event == ABCDK_STCP_EVENT_CONNECT)
    {
        _abcdk_hsvr_event_connect(node);
    }
    ...
    {
        _abcdk_hsvr_event_output(node);
    }
    else if (event == ABCDK_STCP_EVENT_CLOSE || event == ABCDK_STCP_EVENT_INTERRUPT)
    {
        _abcdk_hsvr_event_close(node);
    }
}


static void _abcdk_hsvr_input_cb(abcdk_stcp_node_t *node, const void *data, size_t size, size_t *remain)
{
    abcdk_hsvr_node_t *node_ctx_p;
    int chk;
    ...
    *remain = 0;
    ...
    if (node_ctx_p->protocol == 1)
    {
        _abcdk_hsvr_request_1(node, data, size, remain);
    }
    else if (node_ctx_p->protocol == 2)
    {
        _abcdk_hsvr_request_2(node, data, size, remain);
    }
}

void abcdk_hsvr_session_unref(abcdk_hsvr_session_t **session)
{
    abcdk_stcp_unref((abcdk_stcp_node_t**)session);
}

abcdk_hsvr_session_t *abcdk_hsvr_session_refer(abcdk_hsvr_session_t *src)
{
    return (abcdk_hsvr_session_t*)abcdk_stcp_refer((abcdk_stcp_node_t*)src);
}

abcdk_hsvr_session_t *abcdk_hsvr_session_alloc(abcdk_hsvr_t *ctx)
{
    abcdk_stcp_node_t *node_p;
    abcdk_hsvr_node_t *node_ctx_p;
    abcdk_object_t *stream_p;
    ...
    node_p = abcdk_stcp_alloc(ctx->io_ctx, sizeof(abcdk_hsvr_node_t), _abcdk_hsvr_node_destroy_cb);
    if (!node_p)
        return NULL;
    ...
    node_ctx_p->stream_map->construct_cb = _abcdk_hsvr_stream_construct_cb;
    node_ctx_p->stream_map->destructor_cb = _abcdk_hsvr_stream_destructor_cb;
    node_ctx_p->stream_map->remove_cb = _abcdk_hsvr_stream_remove_cb;
    node_ctx_p->stream_map->opaque = node_p;

    return (abcdk_hsvr_session_t*)node_p;

ERR:

    abcdk_hsvr_session_unref((abcdk_hsvr_session_t**)&node_p);

    return NULL;
}

void *abcdk_hsvr_session_get_userdata(abcdk_hsvr_session_t *session)
{
    abcdk_stcp_node_t *node_p;
    abcdk_hsvr_node_t *node_ctx_p;
...
void *abcdk_hsvr_session_set_userdata(abcdk_hsvr_session_t *session,void *userdata)
{
    abcdk_stcp_node_t *node_p;
    abcdk_hsvr_node_t *node_ctx_p;
    void *old_userdata;
...
const char *abcdk_hsvr_session_get_address(abcdk_hsvr_session_t *session, int remote)
{
    abcdk_stcp_node_t *node_p;
    abcdk_hsvr_node_t *node_ctx_p;
    void *old_userdata;
...
void abcdk_hsvr_session_set_timeout(abcdk_hsvr_session_t *session,time_t timeout)
{
    abcdk_stcp_node_t *node_p;
    abcdk_hsvr_node_t *node_ctx_p;
    void *old_userdata;
...
int abcdk_hsvr_session_listen(abcdk_hsvr_session_t *session,abcdk_sockaddr_t *addr,abcdk_hsvr_config_t *cfg)
{
    abcdk_stcp_node_t *node_p;
    abcdk_hsvr_node_t *node_ctx_p;
    abcdk_hsvr_stream_internal_t *stream_ctx_p;
    ...
    asio_cfg.prepare_cb = _abcdk_hsvr_prepare_cb;
    asio_cfg.event_cb = _abcdk_hsvr_event_cb;
    asio_cfg.input_cb = _abcdk_hsvr_input_cb;
...
void abcdk_hsvr_destroy(abcdk_hsvr_t **ctx)
{
    abcdk_hsvr_t *ctx_p;
...
abcdk_hsvr_t *abcdk_hsvr_create()
{
    abcdk_hsvr_t *ctx;
    int chk;

    ctx = (abcdk_hsvr_t *)abcdk_heap_alloc(sizeof(abcdk_hsvr_t));
    if (!ctx)
        return NULL;
...
ERR:
    ...
    abcdk_hsvr_destroy(&ctx);

    return NULL;
}

/** 释放流.*/
void abcdk_hsvr_unref(abcdk_hsvr_stream_t **stream)
{
    abcdk_object_t *stream_p;
...
abcdk_hsvr_stream_t *abcdk_hsvr_refer(abcdk_hsvr_stream_t *src)
{
    abcdk_object_t *stream_p;
    ...
    return (abcdk_hsvr_stream_t *)abcdk_object_refer(stream_p);
}

abcdk_hsvr_session_t *abcdk_hsvr_get_session(abcdk_hsvr_stream_t *stream)
{
    abcdk_hsvr_node_t *node_ctx_p;
    abcdk_hsvr_stream_internal_t *stream_ctx_p;
    abcdk_object_t *stream_p;
    ...
    return (abcdk_hsvr_session_t*)stream_ctx_p->io_node;
}

void *abcdk_hsvr_get_userdata(abcdk_hsvr_stream_t *stream)
{
    abcdk_hsvr_node_t *node_ctx_p;
    abcdk_hsvr_stream_internal_t *stream_ctx_p;
    abcdk_object_t *stream_p;
...
void *abcdk_hsvr_set_userdata(abcdk_hsvr_stream_t *stream,void *userdata)
{
    abcdk_hsvr_node_t *node_ctx_p;
    abcdk_hsvr_stream_internal_t *stream_ctx_p;
    abcdk_object_t *stream_p;
...
const char *abcdk_hsvr_request_header_get(abcdk_hsvr_stream_t *stream, const char *key)
{
    abcdk_hsvr_node_t *node_ctx_p;
    abcdk_hsvr_stream_internal_t *stream_ctx_p;
    abcdk_object_t *stream_p;
...
const char* abcdk_hsvr_request_header_getline(abcdk_hsvr_stream_t *stream,int line)
{
    abcdk_hsvr_node_t *node_ctx_p;
    abcdk_hsvr_stream_internal_t *stream_ctx_p;
    abcdk_object_t *stream_p;
...
const char *abcdk_hsvr_request_body_get(abcdk_hsvr_stream_t *stream, size_t *len)
{
    abcdk_hsvr_node_t *node_ctx_p;
    abcdk_hsvr_stream_internal_t *stream_ctx_p;
    abcdk_object_t *stream_p;
...
static int _abcdk_hsvr_rsp_hdr_dump_cb(const char *key, const char *value, void *opaque)
{
    abcdk_hsvr_stream_internal_t *stream_ctx_p = (abcdk_hsvr_stream_internal_t *)opaque;
    abcdk_hsvr_node_t *node_ctx_p;
    int chk;
...
static int _abcdk_hsvr_response_header_h1(abcdk_hsvr_stream_t *stream)
{
    abcdk_hsvr_node_t *node_ctx_p;
    abcdk_hsvr_stream_internal_t *stream_ctx_p;
    abcdk_object_t *stream_p;
    ...
    it.dump_cb = _abcdk_hsvr_rsp_hdr_dump_cb;
...
static int _abcdk_hsvr_response_header_h2(abcdk_hsvr_stream_t *stream)
{
#ifdef NGHTTP2_H
    abcdk_hsvr_node_t *node_ctx_p;
    abcdk_hsvr_stream_internal_t *stream_ctx_p;
    ...
    data_prd.read_callback = _abcdk_hsvr_h2_response_read_cb;
...
static int _abcdk_hsvr_response_header(abcdk_hsvr_stream_t *stream)
{
    abcdk_hsvr_node_t *node_ctx_p;
    abcdk_hsvr_stream_internal_t *stream_ctx_p;
    abcdk_object_t *stream_p;
    ...
    if (node_ctx_p->protocol == 1)
    {
        chk = _abcdk_hsvr_response_header_h1(stream);
        if(chk != 0)
            return -1;
    }
    else if (node_ctx_p->protocol == 2)
    {
        chk = _abcdk_hsvr_response_header_h2(stream);
        if(chk != 0)
            return -1;
    }
...
static int _abcdk_hsvr_response_body_h1(abcdk_hsvr_stream_t *stream, abcdk_object_t *data)
{
    abcdk_hsvr_node_t *node_ctx_p;
    abcdk_hsvr_stream_internal_t *stream_ctx_p;
    abcdk_object_t *stream_p;
...
static int _abcdk_hsvr_response_body_h2(abcdk_hsvr_stream_t *stream, abcdk_object_t *data)
{
    abcdk_hsvr_node_t *node_ctx_p;
    abcdk_hsvr_stream_internal_t *stream_ctx_p;
    abcdk_object_t *stream_p;
...
static int _abcdk_hsvr_response_body(abcdk_hsvr_stream_t *stream, abcdk_object_t *data)
{
    abcdk_hsvr_node_t *node_ctx_p;
    abcdk_hsvr_stream_internal_t *stream_ctx_p;
    abcdk_object_t *stream_p;
    ...
    if (node_ctx_p->protocol == 1)
    {
        chk = _abcdk_hsvr_response_body_h1(stream,data);
        if(chk != 0)
            return -1;
    }
    else if (node_ctx_p->protocol == 2)
    {
        chk = _abcdk_hsvr_response_body_h2(stream,data);
        if(chk != 0)
            return -2;
    }
...
void abcdk_hsvr_response_ready(abcdk_hsvr_stream_t *stream)
{
    abcdk_hsvr_node_t *node_ctx_p;
    abcdk_hsvr_stream_internal_t *stream_ctx_p;
    abcdk_object_t *stream_p;
...
int abcdk_hsvr_response_header_vset(abcdk_hsvr_stream_t *stream,const char *key, const char *val, va_list ap)
{
    abcdk_hsvr_node_t *node_ctx_p;
    abcdk_hsvr_stream_internal_t *stream_ctx_p;
    abcdk_object_t *stream_p;
...
int abcdk_hsvr_response_header_set(abcdk_hsvr_stream_t *stream, const char *key, const char *val, ...)
{
    int chk;
    assert(stream != NULL && key != NULL && val != NULL);
    ...
    va_list ap;
    va_start(ap, val);
    chk = abcdk_hsvr_response_header_vset(stream, key, val, ap);
    va_end(ap);

    return chk;
}

void abcdk_hsvr_response_header_unset(abcdk_hsvr_stream_t *stream,const char *key)
{
    abcdk_hsvr_stream_internal_t *stream_ctx_p;
    abcdk_object_t *stream_p;
...
int abcdk_hsvr_response_header_end(abcdk_hsvr_stream_t *stream)
{
    abcdk_hsvr_node_t *node_ctx_p;
    abcdk_hsvr_stream_internal_t *stream_ctx_p;
    abcdk_object_t *stream_p;
    ...
    if(!chk)
    {
        chk = abcdk_hsvr_response_header_set(stream, "Transfer-Encoding", "chunked");
        if(chk != 0)
            return -2;
    }
    ...
    chk = _abcdk_hsvr_response_header(stream);
    if (chk != 0)
        return -3;

    _abcdk_hsvr_log(stream_p,status);

    return 0;
}

int abcdk_hsvr_response(abcdk_hsvr_stream_t *stream, abcdk_object_t *data)
{
    abcdk_hsvr_node_t *node_ctx_p;
    abcdk_hsvr_stream_internal_t *stream_ctx_p;
    abcdk_object_t *stream_p;
    ...
    {
        chk = abcdk_hsvr_response_header_end(stream);
        if(chk != 0)
            return chk;
    }

    chk = _abcdk_hsvr_response_body(stream, data);
    if (chk != 0)
        return -4;
...
int abcdk_hsvr_response_buffer(abcdk_hsvr_stream_t *stream,const void *data, size_t size)
{
    abcdk_object_t *obj;
    int chk;
    ...
    chk = abcdk_hsvr_response(stream,obj);
    if(chk == 0)
        return 0;
...
int abcdk_hsvr_response_format(abcdk_hsvr_stream_t *stream,int max, const char *fmt, ...)
{
    int chk;
    ...
    va_list ap;
    va_start(ap, fmt);
    chk = abcdk_hsvr_response_vformat(stream,max,fmt,ap);
    va_end(ap);

    return chk;
}

int abcdk_hsvr_response_vformat(abcdk_hsvr_stream_t *stream,int max, const char *fmt, va_list ap)
{
    abcdk_object_t *obj;
    int chk;
...
static int _abcdk_hsvr_load_auth(void *opaque, const char *user, char pawd[160])
{
    abcdk_hsvr_node_t *node_ctx_p;
    char tmp[PATH_MAX] = {0};
    int chk;

    node_ctx_p = (abcdk_hsvr_node_t *)opaque;
...
int abcdk_hsvr_check_auth(abcdk_hsvr_stream_t *stream)
{
    abcdk_hsvr_node_t *node_ctx_p;
    abcdk_hsvr_stream_internal_t *stream_ctx_p;
    abcdk_object_t *stream_p;
    ...
    auth_p = abcdk_hsvr_request_header_get(stream, "Authorization");
    if (!auth_p)
    {
        auth_p = abcdk_hsvr_request_header_get(stream, "Proxy-Authorization");
    ...
    chk = abcdk_http_check_auth(auth_opt,_abcdk_hsvr_load_auth,node_ctx_p);
...
ERR:
    ...
    abcdk_hsvr_response_header_set(stream,"Status","%d",(is_proxy ? 407 : 401));

    if(is_proxy)
        abcdk_hsvr_response_header_set(stream,"Proxy-Authenticate","Digest realm=\"%s\", charset=utf-8, nonce=\"%llu\"",node_ctx_p->cfg.realm,abcdk_rand(0,UINT64_MAX-1));
    else 
        abcdk_hsvr_response_header_set(stream,"WWW-Authenticate","Digest realm=\"%s\", charset=utf-8, nonce=\"%llu\"",node_ctx_p->cfg.realm,abcdk_rand(0,UINT64_MAX-1));

 
    abcdk_hsvr_response(stream,NULL);
