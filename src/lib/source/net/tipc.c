...
static void _abcdk_tipc_event_accept(abcdk_stcp_node_t *node, int *result)
{
    ...
    *result = 0;
    ...
    if (*result != 0)
        abcdk_trace_printf(LOG_INFO, TT("禁止客户端('%s')连接到本机。"), node_ctx_p->remote_addr);
}
...
static void _abcdk_tipc_event_connect(abcdk_stcp_node_t *node)
{
    ...
    abcdk_trace_printf(LOG_INFO, TT("本机(%s)与远端(%s)的连接已建立。"), node_ctx_p->local_addr, node_ctx_p->remote_addr);
...
int abcdk_tipc_connect(abcdk_tipc_t *ctx, const char *location, uint64_t id)
{
    ...
    ABCDK_ASSERT(ctx->cfg.id != id,TT("远端ID不能与本机ID相同。"));

    chk = abcdk_sockaddr_from_string(&addr, location, 1);
    if (chk != 0)
    {
        abcdk_trace_printf(LOG_ERR, TT("远端(ID=%llu,IP='%s')的地址无法识别。"),id, location);
        return -4;
    }
...
static void _abcdk_tipc_process_register_req(abcdk_stcp_node_t *node)
{
    ...
    if(myid == node_ctx_p->father->cfg.id)
    {
        ...
        if(chk == -1)
            abcdk_trace_printf(LOG_WARNING,TT("本机ID(%llu)与远端ID(%llu)相同，不允许注册。"),node_ctx_p->father->cfg.id,node_ctx_p->id);
        else if(chk == -3)
            abcdk_trace_printf(LOG_WARNING,TT("相同的远端ID(%llu)已经注册并且在线，不允许注册。"),node_ctx_p->id);
        else 
            abcdk_trace_printf(LOG_WARNING,TT("其它错误。"));
    }
    else
    {
        abcdk_trace_printf(LOG_WARNING,TT("本机ID(%llu)在远端(ID=%llu)登记错误(ID=%llu)，不允许注册。"),node_ctx_p->father->cfg.id,node_ctx_p->id,myid);
    }
...
static void _abcdk_tipc_process_subscribe(abcdk_stcp_node_t *node)
{
    ...
    abcdk_trace_printf(LOG_INFO,TT("远端(ID=%llu)%s主题(%llu)。"),node_ctx_p->id,(unset?TT("取订"):TT("订阅")), topic);
}
