...
static void _abcdk_stcp_ctx_unref(abcdk_stcp_t **ctx)
{
    ...
    *ctx = NULL;
    ...
    ABCDK_ASSERT(ctx_p->worker_ctx == NULL, TT("销毁前必须先停止。"));
...
ssize_t abcdk_stcp_recv(abcdk_stcp_node_t *node, void *buf, size_t size)
{
    ...
    ABCDK_ASSERT(chk == 0, TT("当前线程没有读权利。"));
...
static int _abcdk_stcp_openssl_verify_result(abcdk_stcp_node_t *node)
{
    ...
    if (cert)
    {
        ...
        if (info)
        {
            abcdk_trace_printf(LOG_DEBUG, TT("远端(%s)的证书信息：\n%s"), remote_addr, info->pstrs[0]);
    ...
    if (node->cfg.pki_ca_file || node->cfg.pki_ca_path)
    {
        ...
        if (chk != X509_V_OK)
        {
            abcdk_trace_printf(LOG_WARNING, TT("远端(%s)的证书验证有错误发生(ssl-errno=%d)。"), remote_addr, chk);
            return -1;
        }
    }
...
static void _abcdk_stcp_openssl_dump_errmsg(abcdk_stcp_node_t *node, unsigned long e)
{
    ...
    abcdk_trace_printf(LOG_INFO, TT("本机(%s)与远端(%s)的连接有错误发生(%s)。"), local_addr, remote_addr, errmsg);
}
...
static int _abcdk_stcp_handshake_ssl_init(abcdk_stcp_node_t *node)
{
             ...
             node->cfg.ssl_scheme == ABCDK_STCP_SSL_SCHEME_PKIS)
    {
        if (node->cfg.ssl_scheme == ABCDK_STCP_SSL_SCHEME_RSA || node->cfg.ssl_scheme == ABCDK_STCP_SSL_SCHEME_PKIS)
        {
            ...
            if (!node->openssl_bio)
            {
                abcdk_trace_printf(LOG_WARNING, TT("创建RSA(%d)环境失败。"), node->cfg.ssl_scheme);
                return -1;
            }

            BIO_set_fd(node->openssl_bio, node->fd, BIO_NOCLOSE);
        }
        ...
        if (node->cfg.ssl_scheme == ABCDK_STCP_SSL_SCHEME_PKI || node->cfg.ssl_scheme == ABCDK_STCP_SSL_SCHEME_PKIS)
        {
            ...
            if (!node->openssl_ssl)
            {
                abcdk_trace_printf(LOG_WARNING, TT("创建PKI(%d)环境失败。"), node->cfg.ssl_scheme);
                return -1;
            }
    ...
    {
        abcdk_trace_printf(LOG_WARNING,  TT("尚未支持的SSL配置，或构建时未包含SSL组件。"));
    }
             ...
             node->cfg.ssl_scheme == ABCDK_STCP_SSL_SCHEME_PKIS)
    {
        ...
        if (node->cfg.ssl_scheme == ABCDK_STCP_SSL_SCHEME_PKI || node->cfg.ssl_scheme == ABCDK_STCP_SSL_SCHEME_PKIS)
        {
            ...
            if (node->cfg.pki_next_proto)
            {
                ...
                if (chk != 0)
                {
                    abcdk_trace_printf(LOG_WARNING, TT("设置PKI(%d)环境下层协议和密码套件失败。"), node->cfg.ssl_scheme);
                    return -3;
                }
            }
        }
...
int abcdk_stcp_listen(abcdk_stcp_node_t *node, abcdk_stcp_config_t *cfg)
{
    ...
    ABCDK_ASSERT(cfg->prepare_cb != NULL, TT("未绑定通知回调函数，通讯对象无法正常工作。"));
    ABCDK_ASSERT(cfg->event_cb != NULL, TT("未绑定通知回调函数，通讯对象无法正常工作。"));
    ...
    if(node_p->cfg.bind_ifname && *node_p->cfg.bind_ifname)
    {
        ...
        {
            abcdk_trace_printf(LOG_WARNING,TT("绑定设备需要root权限支持，忽略配置。"));
        }
    }
...
int abcdk_stcp_connect(abcdk_stcp_node_t *node, abcdk_sockaddr_t *addr, abcdk_stcp_config_t *cfg)
{
    ...
    if(node_p->cfg.bind_addr.family) 
    {
        ...
        {
            abcdk_trace_printf(LOG_WARNING, TT("绑定地址的协议和远程的地址协议不同，忽略配置。"));
        }
    }
...
int abcdk_stcp_post(abcdk_stcp_node_t *node, abcdk_object_t *data, int key)
{
    ...
    {
        abcdk_trace_printf(LOG_DEBUG, TT("输出缓慢，队列积压过长(len=%d)，丢弃当前数据包(size=%zd)。\n"), node->out_len, p->obj->sizes[0]);
