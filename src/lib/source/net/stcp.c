...
ssize_t abcdk_stcp_recv(abcdk_stcp_node_t *node, void *buf, size_t size)
{
    ...
    while (rsize_all < size)
    {
        ...
        else if (node->cfg.ssl_scheme == ABCDK_STCP_SSL_SCHEME_PSK)
...
static int _abcdk_stcp_handshake_ssl_init(abcdk_stcp_node_t *node)
{
    if (node->cfg.ssl_scheme == ABCDK_STCP_SSL_SCHEME_RAW)
    {
        return 0;
             ...
             node->cfg.ssl_scheme == ABCDK_STCP_SSL_SCHEME_PSK ||
             node->cfg.ssl_scheme == ABCDK_STCP_SSL_SCHEME_PKIS)
    {
        if (node->cfg.ssl_scheme == ABCDK_STCP_SSL_SCHEME_PSK || node->cfg.ssl_scheme == ABCDK_STCP_SSL_SCHEME_PKIS)
        {
            if (!node->openssl_bio)
                node->openssl_bio = abcdk_openssl_BIO_s_Darknet(node->cfg.psk_use_key->pptrs[0],node->cfg.psk_use_key->sizes[0]);
            else
                return -16;
