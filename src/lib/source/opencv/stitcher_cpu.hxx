    ...
    namespace opencv
    {
        ...
        protected:
            virtual bool remap(const std::vector<abcdk_torch_image_t *> &imgs)
            {
                ...
                if (m_warper_outs.size() != imgs.size())
                {
                    ...
                    for (int i = 0; i < imgs.size(); i++)
                        m_warper_outs.push_back(abcdk_torch_image_alloc(ABCDK_TORCH_TAG_HOST));
                }
                    ...
                    m_owner_warper_ymaps.size() != m_img_good_sizes.size())
                {
                    ...
                    {
                        m_owner_warper_xmaps[i] = abcdk_torch_image_create(m_warper_xmaps[i].cols, m_warper_xmaps[i].rows, ABCDK_TORCH_PIXFMT_GRAYF32, 1);
                        if(!m_owner_warper_xmaps[i])
                            return false;

                        abcdk_torch_image_copy_plane(m_owner_warper_xmaps[i], 0, m_warper_xmaps[i].data, m_warper_xmaps[i].step);

                        m_owner_warper_ymaps[i] = abcdk_torch_image_create(m_warper_ymaps[i].cols, m_warper_ymaps[i].rows, ABCDK_TORCH_PIXFMT_GRAYF32, 1);
                        if(!m_owner_warper_ymaps[i])
                            return false;

                        abcdk_torch_image_copy_plane(m_owner_warper_ymaps[i], 0, m_warper_ymaps[i].data, m_warper_ymaps[i].step);
                    }
                }
                ...
                for (int i = 0; i < m_img_good_idxs.size(); i++)
                {
                    ...
                    /*创建变换后的图像存储空间。*/
                    chk = abcdk_torch_image_reset(&outs_it, warper_w, warper_h, imgs_it->pixfmt, 1);
                    if (chk != 0)
                        return false;

                    abcdk_torch_imgproc_remap(outs_it, NULL, imgs_it, NULL, xmap_it, ymap_it, cv::INTER_CUBIC);
                }
            ...
            virtual bool compose(abcdk_torch_image_t *out, bool optimize_seam = true)
            {
                ...
                /*创建全景图像存储空间。*/
                chk = abcdk_torch_image_reset(&out, m_blend_width, m_blend_height, m_warper_outs[m_img_good_idxs[0]]->pixfmt, 1);
                if (chk != 0)
                    return false;
                ...
                for (int i = 0; i < m_blend_idxs.size(); i++)
                {
                    ...
                    abcdk_torch_imgproc_compose(out, imgs_it, scalar, r.x, r.y, overlap_w, (optimize_seam ? 1 : 0));
                }
