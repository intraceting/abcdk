...
static void _abcdk_ffserver_item_destructor_cb(abcdk_object_t *obj, void *opaque)
{
    ...
    abcdk_trace_printf(LOG_INFO, TT("移除任务(%s)。"), ctx_p->tip);
}
...
int _abcdk_ffserver_src_change_check(abcdk_ffserver_t *ctx)
{
    ...
    for (int i = 0; i < abcdk_ffeditor_streams(src_item_p->ff_ctx); i++)
    {
        ...
        abcdk_trace_printf(LOG_DEBUG, TT("codec_id=%08x,codec_type=%08x,codec_tag=%08x"),
        ...
        // abcdk_trace_printf(LOG_DEBUG, TT("fps=%d"),fps);
        ...
        if(codecpar->codec_type == AVMEDIA_TYPE_VIDEO || codecpar->codec_type == AVMEDIA_TYPE_SUBTITLE)
        {
            ...
            abcdk_trace_printf(LOG_DEBUG, TT("width=%d,height=%d"),width,height);
        }
