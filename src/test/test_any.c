...
#pragma omp parallel for num_threads(4)
    ...
    uint64_t a = abcdk_time_clock2kind_with(CLOCK_REALTIME,9);
...
//#pragma omp parallel for num_threads(4)
 ...
 //   int corner[4] = {100,100,1920,1080};
  ...
  //  abcdk_torch_imgproc_drawrect(a,scalar2,500000,corner);
    ...
    abcdk_torch_imgproc_warp(f, NULL, dst_quad, a, NULL, NULL, 1, 1);
    ...
    snprintf(buf,1000,"otpauth://totp/ABCDK:haha@hehe.com?issuer=ABCDK&secret=%s",en_key->pstrs[0]);

    abcdk_qrcode_encode_save("/tmp/test-totp.bmp",buf,strlen(buf),QR_ECLEVEL_L,10,4);
