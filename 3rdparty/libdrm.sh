...
{ 
    if [ ${FLAG} -eq 1 ];then
        ...
        PackageConfig  --cflags libdrm libdrm_intel libdrm_nouveau libdrm_amdgpu libdrm_radeon 2>/dev/null
    elif [ ${FLAG} -eq 3 ];then
        PackageConfig  --libs libdrm libdrm_intel libdrm_nouveau libdrm_amdgpu libdrm_radeon 2>/dev/null
    elif [ ${FLAG} -eq 4 ];then
        echo "libdrm-dev"
    else
        exit 22
