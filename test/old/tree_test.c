...
{
    int src = *ABCDK_PTR2PTR(int, node1->obj->pptrs[0], 0);
    int dst = *ABCDK_PTR2PTR(int, node2->obj->pptrs[0], 0);
    if( src > dst )
        return 1;
    if( src < dst )
        return -1;
