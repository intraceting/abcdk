...
namespace abcdk
{
    namespace stitcher
    {
        ...
        {
        public:
            static int Dump(const char *file, stitcher_general &obj, const char *magic = NULL)
            {
                assert(file != NULL);
            ...
            static int Load(const char *file, stitcher_general &obj, const char *magic = NULL)
            {
        ...
        protected:
            ...
            virtual void remap(std::vector<cv::Mat> &outs, const std::vector<cv::Mat> &imgs)
            {
                assert(imgs.size() > 0);
                ...
                for (int i = 0; i < m_img_good_idxs.size(); i++)
                {
                    ...
                    cv::remap(imgs[idx], outs[idx], warper_xmap, warper_ymap, cv::INTER_CUBIC, cv::BORDER_CONSTANT, cv::Scalar(0, 0, 0));
                }
            }

            virtual void compose_panorama(cv::Mat &out, const std::vector<cv::Mat> &imgs, int compose_kind = 1)
            {
                int stuff[4] = {0};
                ...
                for (int i = 0; i < m_blend_idxs.size(); i++)
                {
                    ...
                    imageproc::compose(out.channels(), true,
                                       out.data, out.cols, out.step, out.rows,
                                       imgs[idx].data, imgs[idx].cols, imgs[idx].step, imgs[idx].rows,
                                       stuff, r.x, r.y, overlap_w, ((compose_kind == 1) ? 1 : 0));
                }
            }
