...
static void _abcdk_stcp_fix_cfg(abcdk_stcp_node_t *node)
{
    ...
    if(node->cfg.out_hook_min_th <= 0)
        node->cfg.out_hook_min_th = 100;
    else 
        node->cfg.out_hook_min_th = ABCDK_CLAMP(node->cfg.out_hook_min_th,100,600);

    if(node->cfg.out_hook_max_th <= 0)
        node->cfg.out_hook_max_th = 200;
    else 
        node->cfg.out_hook_max_th = ABCDK_CLAMP(node->cfg.out_hook_max_th,200,800);
