...
static void _abcdk_ipool_set_addr_pos(abcdk_ipool_t *ctx,abcdk_sockaddr_t *addr,uint64_t pos)
{
    ...
    {
        memcpy(addr->addr6.sin6_addr.__in6_u.__u6_addr8,ctx->addr_b.addr6.sin6_addr.__in6_u.__u6_addr8,12);
...
static int _abcdk_ipool_init(abcdk_ipool_t *ctx)
{
    ...
    {
        /*检测前缀是否相同。*/
        if(memcmp(ctx->addr_b.addr6.sin6_addr.__in6_u.__u6_addr8,ctx->addr_e.addr6.sin6_addr.__in6_u.__u6_addr8,12) != 0)
            return -4;
    }
...
int abcdk_ipool_set_dhcp_range(abcdk_ipool_t *ctx,abcdk_sockaddr_t *begin,abcdk_sockaddr_t *end)
{
    ...
    {
        /*检测前缀是否相同。*/
        if(memcmp(begin->addr6.sin6_addr.__in6_u.__u6_addr8,end->addr6.sin6_addr.__in6_u.__u6_addr8,12) != 0)
            return -22;

        /*检测前缀是否相同(是否在当前池范围内)。*/
        if(memcmp(ctx->addr_b.addr6.sin6_addr.__in6_u.__u6_addr8,begin->addr6.sin6_addr.__in6_u.__u6_addr8,12) != 0)
            return -1;
    }
...
int abcdk_ipool_static_request(abcdk_ipool_t *ctx,abcdk_sockaddr_t *addr)
{
    ...
    {
        /*检测前缀是否相同。*/
        if(memcmp(ctx->addr_b.addr6.sin6_addr.__in6_u.__u6_addr8,addr->addr6.sin6_addr.__in6_u.__u6_addr8,12) != 0)
            return -4;
    }
