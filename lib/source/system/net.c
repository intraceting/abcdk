...
int abcdk_net_down(const char *ifname)
{
    ...
    if (exitcode != 0)
    {
        abcdk_trace_printf( LOG_ERR, "停用IFN(%s)失败(exit=%d,signal=%d)。", ifname, exitcode, sigcode);

        return -2;
    }

    abcdk_trace_printf(LOG_INFO, "停用IFN(%s)完成。", ifname);
...
int abcdk_net_up(const char *ifname)
{
    ...
    if (exitcode != 0)
    {
        abcdk_trace_printf( LOG_ERR, "启用IFN(%s)失败(exit=%d,signal=%d)。", ifname, exitcode, sigcode);

        return -2;
    }

    abcdk_trace_printf( LOG_INFO, "启用IFN(%s)完成。", ifname);
...
int abcdk_net_address_flush(const char *ifname)
{
    ...
    if (exitcode != 0)
    {
        abcdk_trace_printf(LOG_ERR, "清除IFN(%s)配置失败(exit=%d,signal=%d)。", ifname, exitcode, sigcode);
        return -2;
    }

    abcdk_trace_printf(LOG_INFO, "清除IFN(%s)配置完成。", ifname);
...
int abcdk_net_route_flush(const char *ifname)
{
    ...
    if (exitcode != 0)
    {
        abcdk_trace_printf( LOG_ERR, "清除IFN(%s)路由配置失败(exit=%d,signal=%d)。", ifname, exitcode, sigcode);
        return -2;
    }

    abcdk_trace_printf( LOG_INFO, "清除IFN(%s)路由配置完成。", ifname);
...
int abcdk_net_route_add(int ver, const char *host, int prefix, const char *gw, int metric, const char *ifname)
{
    ...
    {
        abcdk_trace_printf( LOG_ERR, "添加IPLAN('IPV%d','%s/%d','%s','%d')到IFN(%s)失败(exit=%d,signal=%d)。",
                                ver, net, prefix, gw, metric, ifname, exitcode, sigcode);
        return -2;
    }

    abcdk_trace_printf( LOG_INFO, "添加IPLAN('IPV%d','%s/%d','%s','%d')到IFN(%s)完成。",
...
int abcdk_net_address_add(int ver, const char *host, int prefix, const char *gw, int metric, const char *ifname)
{
    ...
    if (exitcode != 0)
    {
        abcdk_trace_printf( LOG_ERR, "添加IPADDR('IPV%d','%s/%d')到IFN(%s)失败(exit=%d,signal=%d)。",
                                ver, host, prefix, ifname, exitcode, sigcode);
        return -2;
    }

    abcdk_trace_printf( LOG_INFO, "添加IPADDR('IPV%d','%s/%d')到IFN(%s)完成。",
...
int abcdk_net_set_mtu(uint16_t mtu, const char *ifname)
{
    ...
    if (exitcode != 0)
    {
        abcdk_trace_printf( LOG_ERR, "更新IFN(%s)最大传输单元失败(exit=%d,signal=%d)。", ifname, exitcode, sigcode);
        return -2;
    }

    abcdk_trace_printf( LOG_INFO, "更新IFN(%s)最大传输单元完成。", ifname);
...
int abcdk_net_set_txqueuelen(uint16_t len,const char *ifname)
{
    ...
    if (exitcode != 0)
    {
        abcdk_trace_printf( LOG_ERR, "更新IFN(%s)队列长度失败(exit=%d,signal=%d)。", ifname, exitcode, sigcode);
        return -2;
    }

    abcdk_trace_printf( LOG_INFO, "更新IFN(%s)队列长度完成。", ifname);
