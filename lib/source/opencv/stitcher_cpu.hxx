        ...
        protected:
            virtual bool remap(const std::vector<abcdk_torch_image_t *> &imgs)
            {
                ...
                for (int i = 0; i < m_img_good_idxs.size(); i++)
                {
                    ...
                    abcdk_torch_imgproc_remap_8u(outs_it, NULL, imgs_it, NULL, xmap_it, ymap_it, cv::INTER_CUBIC);
                }
            ...
            virtual bool compose(abcdk_torch_image_t *out, bool optimize_seam = true)
            {
                ...
                /*创建全景图像存储空间。*/
                chk = abcdk_torch_image_reset(&out, m_blend_width, m_blend_height, m_warper_outs[0]->pixfmt, 1);
                if (chk != 0)
                    return false;
                ...
                for (int i = 0; i < m_blend_idxs.size(); i++)
                {
                    ...
                    abcdk_torch_imgproc_compose_8u(out, imgs_it, scalar, r.x, r.y, overlap_w, (optimize_seam ? 1 : 0));
                }
