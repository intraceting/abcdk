...
namespace abcdk
{
    ...
    protected:
        void find_feature(const std::vector<cv::Mat> &imgs, const std::vector<cv::Mat> &masks)
        {
            ...
            for (int i = 0; i < imgs.size(); i++)
            {
                ...
                if (getenv("AICONTRIG_STITCHER_FIND_FEATURE_DUMP"))
                {
                    cv::Mat img_tmp;
        ...
        void build_map()
        {
            ...
            for (int i = 0; i < m_img_good_sizes.size(); i++)
            {
                ...
                m_warper_rects[i] = m_rotation_warper->buildMaps(m_img_good_sizes[i], K, m_camera_params[i].R,
                                                                 m_warper_xmaps[i], m_warper_ymaps[i]);
        ...
        void remap(std::vector<cv::Mat> &outs, const std::vector<cv::Mat> &imgs)
        {
            assert(imgs.size() > 0);
            ...
            for (int i = 0; i < m_img_good_idxs.size(); i++)
            {
                ...
                assert(imgs[idx].cols == img_w && imgs[idx].rows == img_h);

                if (outs[idx].empty() || outs[idx].rows != warper_h || outs[idx].cols != warper_w || outs[idx].type() != imgs[idx].type())
                    outs[idx].create(warper_h, warper_w, imgs[idx].type());

                cv::remap(imgs[idx], outs[idx], m_warper_xmaps[i], m_warper_ymaps[i], cv::INTER_CUBIC, cv::BORDER_CONSTANT, cv::Scalar(0, 0, 0));
            }
        }

        void remap(std::vector<IMAGE> &outs, const std::vector<IMAGE> &imgs)
        {
            IMAGE stuff;
            ...
            stuff.CopyForm(cv::Scalar(0, 0, 0));
            ...
            for (int i = 0; i < m_img_good_idxs.size(); i++)
            {
                ...
                aicontrib_image_remap(outs[idx], imgs[idx], m_gpu_warper_xmaps[i], m_gpu_warper_ymaps[i], stuff, AICONTRIB_IMAGE_INTER_CUBIC);
            }
        }

        void compose_panorama(cv::Mat &out, const std::vector<cv::Mat> &imgs, int compose_kind = 1)
        {
            /*如果画布不能复用，则创建新的画布。*/
            if (out.empty() || m_blend_height != out.rows || m_blend_width != out.cols || out.type() != imgs[0].type())
                out = cv::Mat(m_blend_height, m_blend_width, imgs[0].type(), cv::Scalar(0, 0, 0));
            ...
            for (int i = 0; i < m_blend_idxs.size(); i++)
            {
                ...
                assert(imgs[idx].type() == out.type());

                /*计算重叠宽度。*/
                int overlap_w = (i <= 0 ? 0 : (m_blend_rects[i - 1].width + m_blend_rects[i - 1].x - m_blend_rects[i].x));

                ImageProc::compose_left2right(out, imgs[idx], cv::Scalar(0, 0, 0), r.x, r.y, overlap_w, ((compose_kind == 1) ? 1 : 0));
            }
        }

        void compose_panorama(IMAGE &out, const std::vector<IMAGE> &imgs, int compose_kind = 1)
        {
            IMAGE stuff;
            ...
            /*如果画布不能复用，则创建新的画布。*/
            out.Create(m_blend_width, m_blend_height, imgs[m_blend_idxs[0]].Pixfmt());
            ...
            for (int i = 0; i < m_blend_idxs.size(); i++)
            {
                ...
                aicontrib_image_compose_left2right(out, imgs[idx], stuff, r.x, r.y, overlap_w, ((compose_kind == 1) ? 1 : 0));
            }
        }

        template <typename T>
        void build_panorama(T &out, const std::vector<T> &imgs, int compose_kind = 1)
        {
            std::vector<T> remap_imgs;
            ...
            compose_panorama(out, remap_imgs, compose_kind);
        }
    ...
    public:
        ...
        template <typename T>
        void BuildPanorama(T &out, const std::vector<T> &imgs, int compose_kind = 1)
        {
            build_panorama<T>(out, imgs, compose_kind);
        }
        ...
        int Prepare(const std::vector<IMAGE> &imgs, const std::vector<std::vector<cv::Rect>> &masks,
                    float good_threshold = 0.8, float adjuster_threshold = 0.8, const char adjuster_mask[5] = "xxxxx",
                    cv::detail::WaveCorrectKind wave_correct_kind = (cv::detail::WaveCorrectKind)-1)
        {
            std::vector<cv::Mat> cpu_imgs;

            assert(imgs.size() >= 2);

            cpu_imgs.resize(imgs.size());

            for (int i = 0; i < imgs.size(); i++)
                imgs[i].CopyTo(cpu_imgs[i]);

            return Prepare(cpu_imgs, masks, good_threshold, adjuster_threshold, adjuster_mask, wave_correct_kind);
        }
                    ...
                    cv::detail::WaveCorrectKind wave_correct_kind = (cv::detail::WaveCorrectKind)-1)
        {
            ...
            {
                tmp.clear();
                abcdk::string::split(tmp, masks[i].c_str(), delim);
        ...
        int Prepare(const std::vector<IMAGE> &imgs, const std::vector<std::string> &masks, const char *delim = ";",
                    float good_threshold = 0.8, float adjuster_threshold = 0.8, const char adjuster_mask[5] = "xxxxx",
                    cv::detail::WaveCorrectKind wave_correct_kind = (cv::detail::WaveCorrectKind)-1)
        {
            std::vector<cv::Mat> cpu_imgs;
            ...
            return Prepare(cpu_imgs, masks, delim, good_threshold, adjuster_threshold, adjuster_mask, wave_correct_kind);
        }
    };
