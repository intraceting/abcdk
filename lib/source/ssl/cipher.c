...
static abcdk_object_t *_abcdk_cipher_aes_256_cbc_update(abcdk_cipher_t *ctx,const uint8_t *in,int in_len)
{
    ...
    if(ctx->encrypt)
    {
        ...
        */
        src_align_len = abcdk_align(salt_len+4+in_len,block_len);

        src = abcdk_object_alloc2(src_align_len);
        if(!src)
            goto ERR;

        dst = abcdk_object_alloc2(src_align_len + block_len);
        if(!dst)
            goto ERR;
    ...
    else 
    {
        ...
        /*修改指针到明文首部。*/
        dst->pptrs[0] = dst->pptrs[0]+salt_len+4;
        /*修改长度为明文实际长度。*/
        dst->sizes[0] = chk;
