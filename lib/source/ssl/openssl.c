...
static int _abcdk_BIO_read_cb(BIO *bio, char *buf, int len)
{
    ...
    rlen = abcdk_easyssl_read(bio_p->easyssl, buf, len);
    if (rlen < 0)
...
static int _abcdk_BIO_write_cb(BIO *bio, const char *buf, int len)
{
    ...
    slen = abcdk_easyssl_write(bio_p->easyssl, buf, len);
    if (slen < 0)
...
static long _abcdk_BIO_ctrl_cb(BIO *bio, int cmd, long num, void *ptr)
{
    ...
    case BIO_C_SET_FD:
    {
        ...
        if (fd >= 0)
        {
            abcdk_easyssl_set_fd(bio_p->easyssl, fd, 0);
            chk = 1;
        }
        else
        {
    ...
    case BIO_C_GET_FD:
    {
        ABCDK_PTR2I32(ptr, 0) = abcdk_easyssl_get_fd(bio_p->easyssl, 0);
        chk = 1;
    }
    break;
    default:
...
static int _abcdk_BIO_destroy_cb(BIO *bio)
{
    ...
    abcdk_easyssl_destroy(&bio_p->easyssl);
...
BIO *abcdk_BIO_s_easyssl(const char *file, uint32_t scheme, size_t salt)
{
    ...
    bio->easyssl = abcdk_easyssl_create_from_file(file, scheme, salt);
    bio->method = _abcdk_BIO_meth_new(BIO_TYPE_SOURCE_SINK,"easyssl BIO");
...
ERR:
    ...
    if(bio)
    {
        abcdk_easyssl_destroy(&bio->easyssl);
