...
abcdk_torch_image_t *abcdk_cuda_image_alloc()
{
    ...
    ctx = abcdk_torch_image_alloc(ABCDK_TORCH_TAG_CUDA);
    if(!ctx)
        return NULL;
...
int abcdk_cuda_image_copy(abcdk_torch_image_t *dst, const abcdk_torch_image_t *src)
{
    ...
    /*复制图像数据。*/
    chk = abcdk_cuda_imgutil_copy(dst->data, dst->stride, (dst->tag == ABCDK_TORCH_TAG_HOST),
...
void abcdk_cuda_image_copy_plane(abcdk_torch_image_t *dst, int dst_plane, const uint8_t *src_data, int src_stride)
{
    ...
    abcdk_cuda_memcpy_2d(dst->data[dst_plane], dst->stride[dst_plane], 0, 0, (dst->tag == ABCDK_TORCH_TAG_HOST),
...
int abcdk_cuda_image_save(const char *dst, const abcdk_torch_image_t *src)
{
    ...
    {
        tmp_src = abcdk_cuda_image_create(src->width,src->height,ABCDK_TORCH_PIXFMT_BGR24,4);
        if(!tmp_src)
            return -1;
    ...
    chk = abcdk_torch_image_save(dst,src);
    if(chk != 0)
        return -1;
