 ...
 *
 */
#ifndef ABCDK_NVIDIA_GRID_HXX
#define ABCDK_NVIDIA_GRID_HXX
...
namespace abcdk
{
    namespace cuda
    {
        namespace grid
        {
            ABCDK_INVOKE_HOST void make_dim(uint3 *dim, size_t n, size_t b)
            {
                size_t k;
                unsigned int x, y;
            ...
            ABCDK_INVOKE_HOST void make_dim_dim(uint3 dim[2], size_t n, size_t b)
            {
                make_dim(&dim[1], b, 1);
                make_dim(&dim[0], n, dim[1].x * dim[1].y * dim[1].z);
            }

            ABCDK_INVOKE_DEVICE size_t get_tid(size_t thread, size_t block)
            {
                /*thread 1D.*/
                if (thread == 1 && block == 0)
                    return threadIdx.x;
