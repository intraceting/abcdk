...
static int _abcdk_cuda_image_convert_cpu(abcdk_torch_image_t *dst, const abcdk_torch_image_t *src)
{
    ...
    if(!dst_in_host)
    {
        ...
        tmp_dst = abcdk_torch_image_create(dst->width,dst->height,dst->pixfmt,1);
        if(!tmp_dst)
            return -1;
    ...
    chk = abcdk_torch_image_convert(dst,src);
    if (chk != 0)
        return -1;
...
static int _abcdk_cuda_image_convert(abcdk_torch_image_t *dst, const abcdk_torch_image_t *src)
{
    abcdk_torch_image_t *tmp_dst = NULL, *tmp_src = NULL;
    int dst_in_host, src_in_host;
    ...
    if (dst->pixfmt == ABCDK_TORCH_PIXFMT_GRAY8)
    {
        ...
        {
            tmp_dst = abcdk_cuda_image_create(dst->width,dst->height, ABCDK_TORCH_PIXFMT_RGB24,1);
            if(!tmp_dst)
                return -1;
